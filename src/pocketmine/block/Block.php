...
class Block extends Position implements BlockIds, Metadatable{	
	...
	public static function get(int $id, int $meta = 0, Position $pos = null){
		...
		try{
			if(self::$fullList !== null) $block = clone self::$fullList[($id << 4) | $meta];
			else $block = new UnknownBlock($id, $meta);
	...
	public function canNeighborBurn(){
		for($face = 0; $face < 5; ++$face){
			if($this->getSide($face)->getBurnChance() > 0) return true;
		}
		return false;
	}
	...
	public function getDrops(Item $item) : array{
		if(!isset(self::$list[$this->getId()])) return [];
		else return [[$this->getId(), $this->getVariant(), 1]];
	}
	...
	public function getSide($side, $step = 1){
		if($this->isValid()) return $this->getLevel()->getBlock(Vector3::getSide($side, $step));
	...
	public function collidesWithBB(AxisAlignedBB $bb){
		foreach($this->getCollisionBoxes() as $bb2){
			if($bb->intersectsWith($bb2)) return true;
		}
	...
	public function getCollisionBoxes() : array{
		if($this->collisionBoxes === null) $this->collisionBoxes = $this->recalculateCollisionBoxes();
	...
	protected function recalculateCollisionBoxes() : array{
		if(($bb = $this->recalculateBoundingBox()) !== null) return [$bb];
	...
	public function getBoundingBox(){
		if($this->boundingBox === null) $this->boundingBox = $this->recalculateBoundingBox();
	...
	public function setMetadata(string $metadataKey, MetadataValue $newMetadataValue){
		if($this->getLevel() instanceof Level) $this->getLevel()->getBlockMetadata()->setMetadata($this, $metadataKey, $newMetadataValue);
	}

	public function getMetadata(string $metadataKey){
		if($this->getLevel() instanceof Level) return $this->getLevel()->getBlockMetadata()->getMetadata($this, $metadataKey);

		return null;
	}

	public function hasMetadata(string $metadataKey) : bool{
		if($this->getLevel() instanceof Level) return $this->getLevel()->getBlockMetadata()->hasMetadata($this, $metadataKey);

		return false;
	}

	public function removeMetadata(string $metadataKey, Plugin $owningPlugin){
		if($this->getLevel() instanceof Level) $this->getLevel()->getBlockMetadata()->removeMetadata($this, $metadataKey, $owningPlugin);
	}
}