...
class Bed extends Transparent{
	...
	public function setOccupied(bool $occupied = true){
		if($occupied) $this->meta |= self::BITFLAG_OCCUPIED;
		else $this->meta &= ~self::BITFLAG_OCCUPIED;

		$this->getLevel()->setBlock($this, $this, false, false);

		if(($other = $this->getOtherHalf()) !== null and $other->isOccupied() !== $occupied) $other->setOccupied($occupied);
	}
	...
	public static function getOtherHalfSide(int $meta, bool $isHead = false) : int{
		...
		if($isHead) $side = Vector3::getOppositeSide($side);
	...
	public function getOtherHalf(){
		...
		if($other instanceof Bed and $other->getId() === $this->getId() and $other->isHeadPart() !== $this->isHeadPart() and (($other->getDamage() & 0x03) === ($this->getDamage() & 0x03))) return $other;
	...
	public function onActivate(Item $item, Player $player = null){
		...
		if($dimension == Level::DIMENSION_NETHER or $dimension == Level::DIMENSION_END){
			$explosion = new Explosion($this, 7);
			$explosion->explodeA();
			$explosion->explodeB();
			$explosion->explodeC();
			return true;
		...
		if($player !== null){
			...
			if($other === null){
				...
				$player->sendMessage(new TranslationContainer(TextFormat::GRAY . "Вы слишком далеко от кровати!"));
				return true;
			}
