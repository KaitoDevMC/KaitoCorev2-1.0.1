...
abstract class Liquid extends Transparent{
	...
	protected function getFlowDecay(Block $block) : int{
		if($block->getId() !== $this->getId()) return -1;
	...
	public function getFlowVector() : Vector3{
		if($this->flowVector !== null) return $this->flowVector;
	...
	public function onUpdate($type){
		if($type === Level::BLOCK_UPDATE_NORMAL){
			...
			if($decay > 0){
				...
				if($newDecay >= 8 or $smallestFlowDecay < 0) $newDecay = -1;

				if(($topFlowDecay = $this->getFlowDecay($this->level->getBlockAt($this->x, $this->y + 1, $this->z))) >= 0) $newDecay = $topFlowDecay | 0x08;

				if($this->adjacentSources >= 2 and $this instanceof Water){
					$bottomBlock = $this->level->getBlockAt($this->x, $this->y - 1, $this->z);
					if($bottomBlock->isSolid()) $newDecay = 0;
					elseif($bottomBlock instanceof Water and $bottomBlock->getDamage() === 0) $newDecay = 0;
				}
			...
			if($decay >= 0){
				...
				if($decay === 0 or !$bottomBlock->canBeFlowedInto()){
					if($decay >= 8) $adjacentDecay = 1;
					else $adjacentDecay = $decay + $multiplier;

					if($adjacentDecay < 8){
						$flags = $this->getOptimalFlowDirections();

						if($flags[0]) $this->flowIntoBlock($this->level->getBlockAt($this->x - 1, $this->y, $this->z), $adjacentDecay);

						if($flags[1]) $this->flowIntoBlock($this->level->getBlockAt($this->x + 1, $this->y, $this->z), $adjacentDecay);
						if($flags[2]) $this->flowIntoBlock($this->level->getBlockAt($this->x, $this->y, $this->z - 1), $adjacentDecay);
	...
	protected function flowIntoBlock(Block $block, int $newFlowDecay) : void{
		if($this->canFlowInto($block) and !($block instanceof Liquid)){
			if($block instanceof Lava) $this->triggerLavaMixEffects($block);
			elseif($block->getId() > 0) $this->level->useBreakOn($block);
	...
	private function calculateFlowCost(int $blockX, int $blockY, int $blockZ, int $accumulatedCost, int $maxCost, int $originOpposite, int $lastOpposite) : int{
		...
		for($j = 0; $j < 4; ++$j){
			if($j === $originOpposite or $j === $lastOpposite) continue;
			...
			if(!isset($this->flowCostVisited[$hash = Level::blockHash($x, $y, $z)])){
				...
				if(!$this->canFlowInto($blockSide)) $this->flowCostVisited[$hash] = self::BLOCKED;
				elseif($this->level->getBlockAt($x, $y - 1, $z)->canBeFlowedInto()) $this->flowCostVisited[$hash] = self::CAN_FLOW_DOWN;
				else $this->flowCostVisited[$hash] = self::CAN_FLOW;
			}

			$status = $this->flowCostVisited[$hash];

			if($status === self::BLOCKED) continue;
			elseif($status === self::CAN_FLOW_DOWN) return $accumulatedCost;
	...
	private function getOptimalFlowDirections() : array{
		...
		for($i = 0; $i < 4; ++$i) $isOptimalFlowDirection[$i] = ($flowCost[$i] === $minCost);
	...
	protected function triggerLavaMixEffects(Vector3 $pos){
		...
		for($i = 0; $i < 8; ++$i) $this->level->addParticle(new SmokeParticle($pos->add(mt_rand(0, 80) / 100, 0.5, mt_rand(0, 80) / 100)));
	}
}
