...
class Cauldron extends Solid {
	...
	public function onActivate(Item $item, Player $player = null){//@author iTX. rewrite @Dog194
		...
		if(!($tile instanceof TileCauldron)) return false;
		switch($item->getId()){
			case Item::BUCKET:
				if($item->getDamage() === 0){//empty bucket
					if(!$this->isFull() or $tile->isCustomColor() or $tile->hasPotion()) break;
					$bucket = clone $item;
					...
					if(!$ev->isCancelled()){
						if($player->isSurvival()) $player->getInventory()->setItemInHand($ev->getItem());
						$this->meta = 0;//empty
			...
			case Item::DYE:
				...
				if($tile->isCustomColor()) $color = Color::averageColor($color, $tile->getCustomColor());
				if($player->isSurvival()) $item->setCount($item->getCount() - 1);
			...
			case Item::GLASS_BOTTLE:
				...
				if($tile->hasPotion()){
					$this->meta -= 2;
					if($tile->getSplashPotion() === true) $result = Item::get(Item::SPLASH_POTION, $tile->getPotionId());
					else $result = Item::get(Item::POTION, $tile->getPotionId());
					if($this->isEmpty()){
						$tile->setPotionId(0xffff);//reset
						$tile->setSplashPotion(false);
						$tile->clearCustomColor();
