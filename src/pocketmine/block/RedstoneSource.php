...
class RedstoneSource extends Flowable {
	...
	public function activateBlockWithoutWire(Block $block){
		...
		switch($block->getId()){
			case Block::TNT: $block->onActivate(new Item(Item::FLINT_AND_STEEL)); break;
			case Block::REDSTONE_LAMP: $block->turnOn(); break;
			case Block::DROPPER:
			case Block::DISPENSER:
				$block->activate();
			break;
			case Block::UNPOWERED_REPEATER_BLOCK:
	...
	public function activateBlock(Block $block){
		...
		if($block->getId() == Block::REDSTONE_WIRE){
			...
			$block->calcSignal($this->maxStrength, RedstoneWire::ON);
		}
	}
	...
	public function deactivateBlock(Block $block){
		...
		if($block->getId() == Block::REDSTONE_WIRE) $block->calcSignal(0, RedstoneWire::OFF);
	}
	...
	public function deactivate(array $ignore = []){
		if($this->canCalc()){
			...
			foreach($sides as $side){
				if(!in_array($side, $ignore)) $this->deactivateBlock($this->getSide($side));
			}
	...
	public function checkPower(Block $block, array $ignore = [], $ignoreWire = false){
		if($block instanceof PoweredRepeater){
			if($block->getSide($block->getDirection())->isActivated($block)) return true;
			return false;
		}
		$sides = [
			Vector3::SIDE_EAST,
