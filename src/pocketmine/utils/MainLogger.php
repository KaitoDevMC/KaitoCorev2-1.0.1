...
class MainLogger extends \AttachableThreadedLogger{
	...
	public function __construct(string $logFile, bool $logDebug = false){
		parent::__construct();
		if(static::$logger instanceof MainLogger) throw new \RuntimeException("MainLogger has been already created");
		touch($logFile);
	...
	public function debug($message, bool $force = false){
		if(!$this->logDebug and !$force) return;
	...
	public function logException(\Throwable $e, $trace = null){
		...
		$this->synchronized(function() use ($e, $trace) : void{
			...
			foreach(Utils::printableTrace($trace) as $line) $this->critical($line);
			for($prev = $e->getPrevious(); $prev !== null; $prev = $prev->getPrevious()){
				$this->critical("Previous: " . self::printExceptionMessage($prev));
				foreach(Utils::printableTrace($prev->getTrace()) as $line) $this->critical("  " . $line);
			}
		});
	...
	private static function printExceptionMessage(\Throwable $e) : string{
		...
		return get_class($e) . ": \"$errstr\" ($errno) in \"".Utils::cleanPath($e->getFile())."\" at line ".$e->getLine();
	}
	...
	protected function send($message, $level, $prefix, $color){
		...
		static $time = null;
		if($time === null) $time = new \DateTime('now', new \DateTimeZone($this->timezone));
		...
		}else $threadName = (new \ReflectionClass($thread))->getShortName() . " thread";

		$message = sprintf($this->format, $time->format("H:i:s"), $color, $threadName, $prefix, TextFormat::clean($message, false));

		if(!Terminal::isInit()) Terminal::init($this->mainThreadHasFormattingCodes); //lazy-init colour codes because we don't know if they've been registered on this thread

		if(isset($this->consoleCallback)) call_user_func($this->consoleCallback);

		$this->synchronized(function() use ($message, $level, $time) : void{
			Terminal::writeLine($message);

			foreach($this->attachments as $attachment) $attachment->call($level, $message);
	...
	public function syncFlushBuffer(){
		...
		$this->synchronized(function() : void{
			while($this->syncFlush) $this->wait(); //block until it's all been written to disk
		});
	}
	...
	private function writeLogStream($logResource) : void{
		while($this->logStream->count() > 0){
			fwrite($logResource, $this->logStream->shift());
		}
	...
	public function run(){
		...
		if(!is_resource($logResource)) throw new \RuntimeException("Couldn't open log file");

		while(!$this->shutdown){
			if($this->write) $this->writeLogStream($logResource);
			$this->synchronized(function() : void{
				if(!$this->shutdown && !$this->syncFlush) $this->wait();
			});
		}
