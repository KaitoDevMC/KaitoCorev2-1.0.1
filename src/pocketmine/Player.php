...
use pocketmine\level\sound\{MilkSound, ShearSound};
...
use pocketmine\event\player\PlayerFallEvent;
...
/**
 * Main class that handles networking, recovery, and packet sending to the server partbounding
 */
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer {
	...
	public function setFishingHook(FishingHook $entity = null){
		if($entity === null and $this->fishingHook instanceof FishingHook) $this->fishingHook->close();

		if($entity !== null) $entity->setOwningEntity($this);
	...
	public function setWhitelisted($value){
		if($value) $this->server->addWhitelist($this->getLowerCaseName());
		else $this->server->removeWhitelist($this->getLowerCaseName());
	}
	...
	public function spawnTo(Player $player){
		if($this->spawned and $player->spawned and $this->isAlive() and $player->isAlive() and $player->getLevel() === $this->level and $player->canSee($this) and !$this->isSpectator()) parent::spawnTo($player);
	}
	...
	public function setOp($value){
		if($value) $this->server->addOp($this->getName());
	...
	public function removeAchievement($achievementId){
		if($this->hasAchievement($achievementId)) $this->achievements[$achievementId] = false;
	}
	...
	public function getSpawn(){
		if($this->hasValidSpawnPosition()) return $this->spawnPosition;
		else return $this->server->getDefaultLevel()->getSafeSpawn();
	}
	...
	public function sendChunk(int $x, int $z, BatchPacket $payload){
		...
		if($this->spawned){
			foreach($this->level->getChunkEntities($x, $z) as $entity){
				if($entity !== $this and !$entity->closed and $entity->isAlive()) $entity->spawnTo($this);
			}
		}

		if($this->spawnChunkLoadCount !== -1 and ++$this->spawnChunkLoadCount >= $this->spawnThreshold) $this->doFirstSpawn();
	}
	...
	public function sendPing(){
		if($this->ping <= 150) return $this->sendMessage("§f► §eОтличное соединение §7(§a§l{$this->ping}ms§r§7)");
		if($this->ping <= 250) return $this->sendMessage("§f► §eХорошее соединение §7(§e§l{$this->ping}ms§r§7)");
		$this->sendMessage("§f► §eУжасное соединение §7(§c§l{$this->ping}ms§r§7)");
	}
	...
	protected function sendNextChunk(){
		...
		foreach($this->loadQueue as $index => $distance){
			...
			if(!$this->level->populateChunk($X, $Z)) continue;

			unset($this->loadQueue[$index]);
			$this->level->requestChunk($X, $Z, $this);
		}

		if($this->spawnChunkLoadCount >= $this->spawnThreshold and !$this->spawned) $this->doFirstSpawn();
	...
	protected function doFirstSpawn(){
		...
		if($this->getHealth() <= 0) $this->respawn();
	...
	protected function orderChunks(){
		...
		for($x = 0; $x < $radius; ++$x){
			for($z = 0; $z <= $x; ++$z){
				if(($x ** 2 + $z ** 2) > $radiusSquared) break; //skip to next band

				//If the chunk is in the radius, others at the same offsets in different quadrants are also guaranteed to be.

				/* Top right quadrant */
				if(!isset($this->usedChunks[$index = Level::chunkHash($centerX + $x, $centerZ + $z)]) or !$this->usedChunks[$index]) $newOrder[$index] = true;
				unset($unloadChunks[$index]);

				/* Top left quadrant */
				if(!isset($this->usedChunks[$index = Level::chunkHash($centerX - $x - 1, $centerZ + $z)]) or !$this->usedChunks[$index]) $newOrder[$index] = true;
				unset($unloadChunks[$index]);

				/* Bottom right quadrant */
				if(!isset($this->usedChunks[$index = Level::chunkHash($centerX + $x, $centerZ - $z - 1)]) or !$this->usedChunks[$index]) $newOrder[$index] = true;
				unset($unloadChunks[$index]);


				/* Bottom left quadrant */
				if(!isset($this->usedChunks[$index = Level::chunkHash($centerX - $x - 1, $centerZ - $z - 1)]) or !$this->usedChunks[$index]) $newOrder[$index] = true;
				unset($unloadChunks[$index]);

				if($x !== $z){
					/* Top right quadrant mirror */
					if(!isset($this->usedChunks[$index = Level::chunkHash($centerX + $z, $centerZ + $x)]) or !$this->usedChunks[$index]) $newOrder[$index] = true;
					unset($unloadChunks[$index]);

					/* Top left quadrant mirror */
					if(!isset($this->usedChunks[$index = Level::chunkHash($centerX - $z - 1, $centerZ + $x)]) or !$this->usedChunks[$index]) $newOrder[$index] = true;
					unset($unloadChunks[$index]);

					/* Bottom right quadrant mirror */
					if(!isset($this->usedChunks[$index = Level::chunkHash($centerX + $z, $centerZ - $x - 1)]) or !$this->usedChunks[$index]) $newOrder[$index] = true;
					unset($unloadChunks[$index]);

					/* Bottom left quadrant mirror */
					if(!isset($this->usedChunks[$index = Level::chunkHash($centerX - $z - 1, $centerZ - $x - 1)]) or !$this->usedChunks[$index]) $newOrder[$index] = true;
					unset($unloadChunks[$index]);
				}
			}
		}
	...
	public function dataPacket(DataPacket $packet, bool $needACK = false, bool $immediate = false){
		...
		if(!$this->loggedIn and !$packet->canBeSentBeforeLogin()) throw new \InvalidArgumentException("Attempted to send " . get_class($packet) . " to " . $this->getName() . " too early");
	...
	public function sleepOn(Vector3 $pos){
		...
		if($b instanceof Bed) $b->setOccupied();
	...
	public function setSpawn(Vector3 $pos){
		if(!($pos instanceof Position)) $level = $this->level;
	...
	public function stopSleep(){
		if($this->sleeping instanceof Vector3){
			...
			if($b instanceof Bed) $b->setOccupied(false);
	...
	public function awardAchievement($achievementId){
		if(isset(Achievement::$list[$achievementId]) and !$this->hasAchievement($achievementId)){
			foreach(Achievement::$list[$achievementId]["requires"] as $requirementId){
				if(!$this->hasAchievement($requirementId)) return false;
			}
	...
	public static function getClientFriendlyGamemode(int $gamemode) : int{
		$gamemode &= 0x03;
		if($gamemode === Player::SPECTATOR) return Player::CREATIVE;
	...
	public function setGamemode(int $gm, bool $client = false){
		...
		if($this->isSpectator()){
			...
			if($this->isSurvival()) $this->setFlying(false);
			$this->spawnToAll();
		}
		...
		if(!$client){ //Gamemode changed by server, do not send for client changes
			$this->sendGamemode();
		}else Command::broadcastCommandMessage($this, new TranslationContainer("commands.gamemode.success.self", [Server::getGamemodeString($gm)]));
	...
	public function getDrops(){
		if(!$this->isCreative()) return parent::getDrops();
	...
	public function startFishing(){
        $fishingHook = new FishingHook($this->level, Entity::createBaseNBT( $this->add(0, $this->getEyeHeight(), 0),new Vector3(-sin(deg2rad($this->yaw)) * cos(deg2rad($this->pitch)) * 0.9 * 0.9,-sin(deg2rad($this->pitch)) * 0.9 * 0.9,cos(deg2rad($this->yaw)) * cos(deg2rad($this->pitch)) * 0.9 * 0.9),$this->yaw,$this->pitch), $this);
	...
	protected function checkNearEntities(){
		foreach($this->level->getNearbyEntities($this->boundingBox->grow(0.3, 0.3, 0.3), $this) as $entity){
			...
			if($entity instanceof Arrow and $entity->hadCollision and $entity->getPotionId() !== 0){
			...
			}elseif($entity instanceof MinecartTNT){
				if($this->getLevel()->getBlock(new Vector3($entity->getX(), $entity->getY(), $entity->getZ()))->getId() == 66) $entity->setMotion($this->getDirectionVector()->multiply(0.5));
				else $entity->setMotion($this->getDirectionVector()->multiply(0.04));
			}elseif($entity instanceof XPOrb){
				$this->getLevel()->getServer()->getPluginManager()->callEvent($ev = new PlayerPickupExpOrbEvent($this, $entity->getExperience()));
				if($ev->isCancelled()) break;
				$this->level->addSound(new ExpPickupSound($this, mt_rand(0, 1000)));
				$entity->setMotion(new Vector3(50, 50, 50));
				$entity->kill();
				$entity->close();
				if($this->getItemInHand()->hasEnchantment(26) and $this->getItemInHand()->getDamage() > 0){
					$ihand = $this->getItemInHand();
					$rand = mt_rand(1, 3);
					if($ihand->getDamage() - $rand < 0) $ihand->setDamage(0);
					else $ihand->setDamage($ihand->getDamage() - $rand);
					$this->getInventory()->setItemInHand($ihand);
					unset($ihand);
					unset($rand);
					break;
				}
				if($this->getInventory()->getHelmet()->getDamage() > 0 and $this->getInventory()->getHelmet()->hasEnchantment(26)){
					$helmet = $this->getInventory()->getHelmet();
					$rand = mt_rand(1, 3);
					if($helmet->getDamage() - $rand < 0) $helmet->setDamage(0);
					else $helmet->setDamage($helmet->getDamage() - $rand);
					$this->getInventory()->setHelmet($helmet);
					unset($helmet);
					unset($rand);
					break;
				}
				if($this->getInventory()->getChestplate()->getDamage() > 0 and $this->getInventory()->getChestplate()->hasEnchantment(26)){
					$chestplate = $this->getInventory()->getChestplate();
					$rand = mt_rand(1, 3);
					if($chestplate->getDamage() - $rand < 0) $chestplate->setDamage(0);
					else $chestplate->setDamage($chestplate->getDamage() - $rand);
					$this->getInventory()->setChestplate($chestplate);
					unset($chestplate);
					unset($rand);
					break;
				}
				if($this->getInventory()->getLeggings()->getDamage() > 0 and $this->getInventory()->getLeggings()->hasEnchantment(26)){
					$leggings = $this->getInventory()->getLeggings();
					$rand = mt_rand(1, 3);
					if($leggings->getDamage() - $rand < 0) $leggings->setDamage(0);
					else $leggings->setDamage($leggings->getDamage() - $rand);
					$this->getInventory()->setLeggings($leggings);
					unset($leggings);
					unset($rand);
					break;
				}
				if($this->getInventory()->getBoots()->getDamage() > 0 and $this->getInventory()->getBoots()->hasEnchantment(26)){
					$boots = $this->getInventory()->getBoots();
					$rand = mt_rand(1, 3);
					if($boots->getDamage() - $rand < 0) $boots->setDamage(0);
					else $boots->setDamage($boots->getDamage() - $rand);
					$this->getInventory()->setBoots($boots);
					unset($boots);
					unset($rand);
					break;
				}
				$this->addXp($entity->getExperience());
    ...
    if(!$this->isAlive() or !$this->spawned or $this->newPosition === null or $this->isSleeping()) return;
    ...
    if(!$revert and $distanceSquared != 0){
      ...
      if($this->distanceSquared($newPos) / $tickDiff ** 2 > 0 and !$revert) $this->setPosition($newPos);
    }
    ...
    if(!$revert and ($delta > 0.0001 or $deltaAngle > 1.0)){
      ...
      if(!($revert = $ev->isCancelled())){ //Yes, this is intended
        ...
        if($to->distanceSquared($ev->getTo()) > 0.01){ //If plugins modify the destination
          ...
          if($this->isSprinting()) $this->exhaust(0.1 * $distance, PlayerExhaustEvent::CAUSE_SPRINTING);
          else $this->exhaust(0.01 * $distance, PlayerExhaustEvent::CAUSE_WALKING);
        }

        if($this->fishingHook instanceof FishingHook){
          if($this->distance($this->fishingHook) > 33 or $this->inventory->getItemInHand()->getId() !== Item::FISHING_ROD) $this->setFishingHook();
        }
        if($this->getInventory()->getBoots()->hasEnchantment(25)){ //Frost Walker
        	$level = $this->getLevel();
        	for($x2 = 0; $x2 <= 3; ++$x2){
        		for($z2 = 0; $z2 <= 3; ++$z2){
        			if($level->getBlock(new Vector3($this->getX() + $x2, $this->getY() - 1, $this->getZ() + $z2))->getId() === 8 or $level->getBlock(new Vector3($this->getX() + $x2, $this->getY() - 1, $this->getZ() + $z2))->getId() === 9) $level->setBlock(new Vector3($this->getX() + $x2, $this->getY() - 1, $this->getZ() + $z2), Block::get(207, 0));
        			if($level->getBlock(new Vector3($this->getX() - $x2, $this->getY() - 1, $this->getZ() - $z2))->getId() === 8 or $level->getBlock(new Vector3($this->getX() - $x2, $this->getY() - 1, $this->getZ() - $z2))->getId() === 9) $level->setBlock(new Vector3($this->getX() - $x2, $this->getY() - 1, $this->getZ() - $z2), Block::get(207, 0));
        			if($level->getBlock(new Vector3($this->getX() - $x2, $this->getY() - 1, $this->getZ() + $z2))->getId() === 8 or $level->getBlock(new Vector3($this->getX() - $x2, $this->getY() - 1, $this->getZ() + $z2))->getId() === 9) $level->setBlock(new Vector3($this->getX() - $x2, $this->getY() - 1, $this->getZ() + $z2), Block::get(207, 0));
        			if($level->getBlock(new Vector3($this->getX() + $x2, $this->getY() - 1, $this->getZ() - $z2))->getId() === 8 or $level->getBlock(new Vector3($this->getX() + $x2, $this->getY() - 1, $this->getZ() - $z2))->getId() === 9) $level->setBlock(new Vector3($this->getX() + $x2, $this->getY() - 1, $this->getZ() - $z2), Block::get(207, 0));
        		}
    ...
    }elseif($distanceSquared != 0 and $this->nextChunkOrderRun > 20) $this->nextChunkOrderRun = 20;
	...
	public function fall($fallDistance) : void{
		if(!$this->flying){
			$this->server->getPluginManager()->callEvent($ev = new PlayerFallEvent($this, $fallDistance));
	...
	public function onUpdate($currentTick){
		...
		if(!$this->isAlive() and $this->spawned){
			++$this->deadTicks;
			if($this->deadTicks >= $this->maxDeadTicks) $this->despawnFromAll();
			return true;
		}
		...
		if($this->spawned){
			if($this->server->netherEnabled){
				if(($this->isCreative() or $this->isSurvival() and $this->server->getTick() - $this->portalTime >= 80) and $this->portalTime > 0){
					$netherLevel = null;
					if($this->server->isLevelLoaded($this->server->netherName) or $this->server->loadLevel($this->server->netherName)) $netherLevel = $this->server->getLevelByName($this->server->netherName);
					...
					if($netherLevel instanceof Level){
						if($this->getLevel()->getName() === 'world'){
							...
							$z = $this->getZ() * 8;
							for($x2 = 0; $x2 < 4; ++$x2){
								for($y2 = 0; $y2 < 5; ++$y2) $netherLevel->setBlock(new Vector3($x - $x2, $y + $y2, $z), Block::get(49));
							}
							for($x2 = 1; $x2 < 3; ++$x2){
								for($y2 = 1; $y2 < 4; ++$y2) $netherLevel->setBlock(new Vector3($x - $x2, $y + $y2, $z), Block::get(90));
							}
							$this->teleport(new Position($this->getX() * 8, $this->getY(), $this->getZ() * 8, $netherLevel));
						}else{
							$this->teleport(new Position($this->getX() / 8, $this->getY(), $this->getZ() / 8, Server::getInstance()->getLevelByName('world')));
						}
						$this->portalTime = 0;
					}
				}
			}
			if($this->getLevel()->getBlock($this->asVector3()->subtract(0, 1, 0))->getId() === 119){ //EndPortal check
				if($this->server->enderEnabled){
					if($this->isCreative() or $this->isSurvival()){
						$enderLevel = null;
						if($this->server->isLevelLoaded($this->server->enderName) or $this->server->loadLevel($this->server->enderName)) $enderLevel = $this->server->getLevelByName($this->server->enderName); //teleport player to ender world
						if($enderLevel instanceof Level){
							if($this->getLevel()->getName() === 'world') $this->teleport(new Position(0, 50, 0, $enderLevel));
							else $this->teleport(Server::getInstance()->getLevelByName('world')->getSafeSpawn()->asPosition());
						}
					}
				}
			}
			...
			if($this->onGround) $this->inAirTicks = 0;
			else $this->inAirTicks += $tickDiff;
			...
			if($this->isOnFire() or $this->lastUpdate % 10 == 0){
				if($this->isCreative() and !$this->isInsideOfFire()){
					...
					if($this->getLevel()->canBlockSeeSky($this)) $this->extinguish();
				}
			}
	...
	public function doFoodTick($tickDiff = 1){
		if($this->isSurvival()) parent::doFoodTick($tickDiff);
	}

	public function exhaust(float $amount, int $cause = PlayerExhaustEvent::CAUSE_CUSTOM) : float{
		if($this->isSurvival()) return parent::exhaust($amount, $cause);
	...
	public function checkNetwork(){
		...
		if(sizeof($this->loadQueue) > 0) $this->sendNextChunk();

		if(sizeof($this->batchedPackets) > 0){
	...
	public function canInteract(Vector3 $pos, float $maxDistance, float $maxDiff = M_SQRT3 / 2){
		...
		return ($dV->dot($pos) - $dV->dot($eyePos)) >= -$maxDiff;
	}
	...
	public function getCreativeItemIndex(Item $item) : int{
		foreach($this->personalCreativeItems as $i => $d){
			if($item->equals($d, !$item->isTool())) return $i;
		}
	...
	protected function processLogin(){
		if(!$this->server->isWhitelisted($this->getLowerCaseName())) return $this->close($this->getLeaveMessage(), "§fНа сервере проходят §aтехнические работы.\n§fСледите за §eновостями §fв нашем сообществе VK.\n§avk.com/rizeminepe");
		if($this->isBanned() or $this->server->getIPBans()->isBanned($this->getAddress()) or $this->server->getCIDBans()->isBanned($this->randomClientId)) return $this->close($this->getLeaveMessage(), TextFormat::RED . "Вы забанены");

		if($this->hasPermission(Server::BROADCAST_CHANNEL_USERS)) $this->server->getPluginManager()->subscribeToPermission(Server::BROADCAST_CHANNEL_USERS, $this);
		if($this->hasPermission(Server::BROADCAST_CHANNEL_ADMINISTRATIVE)) $this->server->getPluginManager()->subscribeToPermission(Server::BROADCAST_CHANNEL_ADMINISTRATIVE, $this);

		foreach($this->server->getOnlinePlayers() as $p){
			if($p !== $this and ($p->iusername === $this->iusername or $this->getUniqueId()->equals($p->getUniqueId()))) return $this->close($this->getLeaveMessage(), "§fИгрок с данным ником уже играет. §aСмените ник!");
		}
		...
		if(!($nbt instanceof CompoundTag)) return $this->close($this->getLeaveMessage(), "Некорректно переданы данные игрока!");
		$this->achievements = [];
		...
		if(!$this->hasValidSpawnPosition()){
			if(isset($this->namedtag->SpawnLevel) and ($level = $this->server->getLevelByName((string) $this->namedtag["SpawnLevel"])) instanceof Level) $this->spawnPosition = new Position($this->namedtag["SpawnX"], $this->namedtag["SpawnY"], $this->namedtag["SpawnZ"], $level);
			else $this->spawnPosition = $this->level->getSafeSpawn();
		}
		...
		if($ev->isCancelled()) return $this->close($this->getLeaveMessage(), $ev->getKickMessage());
		$this->level->sendTime($this);
	...
	public function handleDataPacket(DataPacket $packet){
		...
		if(isset($this->server->getNetwork()->ipSec[$this->getAddress()])){
			if(++$this->server->getNetwork()->ipSec[$this->getAddress()] >= $this->server->getNetwork()->packetLimit){
				$this->server->getNetwork()->blockAddress($this->getAddress(), 300);
				return;
			}
			$this->server->getNetwork()->ipSec[$this->getAddress()] = 1;
		}
		...
		if($ev->isCancelled()) return $timings->stopTiming();

		switch($packet::NETWORK_ID){
            case ProtocolInfo::MAP_INFO_REQUEST_PACKET:
                foreach($this->getInventory()->getContents() as $content){
                    if($content instanceof FilledMap){
                        if($content->getMapId() === $this->uuid) return $this->dataPacket($content->getSavedData());
                    }
                }
            ...
            case ProtocolInfo::LOGIN_PACKET:
				...
				if(!$this->isValidDeviceOS($packet->deviceOS) and $this->close("", "", false)) return $this->server->getNetwork()->blockAddress($this->getAddress(), 3600);
				$pk = new PlayStatusPacket();
				$pk->status = PlayStatusPacket::LOGIN_SUCCESS;
				$this->dataPacket($pk);
				...
				if($this->server->getConfigBoolean("online-mode", false) && $packet->identityPublicKey === null && $this->kick("disconnectionScreen.notAuthenticated", false)) break;

				if(sizeof($this->server->getOnlinePlayers()) >= $this->server->getMaxPlayers() && $this->kick("disconnectionScreen.serverFull", false)) break;
				...
				if(!self::isValidUserName($packet->username) and $this->close("", "disconnectionScreen.invalidName")) return true;
				...
				if((strlen($packet->skin) != 64 * 64 * 4) and (strlen($packet->skin) != 64 * 32 * 4) and $this->close("", "disconnectionScreen.invalidSkin")) break;
				$this->setSkin($packet->skin, $packet->skinId);

				$this->server->getPluginManager()->callEvent($ev = new PlayerPreLoginEvent($this, "Plugin reason"));
				if($ev->isCancelled() and $this->close("", $ev->getKickMessage())) break;
				...
				switch($packet->status){
					...
					case ResourcePackClientResponsePacket::STATUS_SEND_PACKS:
						...
						foreach($packet->packIds as $uuid){
							...
							if(!($pack instanceof ResourcePack) and $this->close("", $this->server->getLanguage()->translateString("disconnectionScreen.unavailableResourcePack"), true)){
					...
					case ResourcePackClientResponsePacket::STATUS_HAVE_ALL_PACKS:
						if(!$this->havePacks){
							...
							$this->server->getNetwork()->blockAddress($this->getAddress(), 3600);
							$this->close('', '');
						}
					break;
			...
			case ProtocolInfo::RESOURCE_PACK_CHUNK_REQUEST_PACKET:
				...
				if(!($pack instanceof ResourcePack) and $this->close("", "disconnectionScreen.resourcePack", true)) return true;
			...
			case ProtocolInfo::MOVE_PLAYER_PACKET:
				if($this->linkedEntity instanceof Entity){
					if($this->linkedEntity instanceof Boat) $this->linkedEntity->setPosition($this->temporalVector->setComponents($packet->x, $packet->y - 0.3, $packet->z));
				}

				$newPos = new Vector3($packet->x, $packet->y - $this->baseOffset, $packet->z);

				if($newPos->distanceSquared($this) < 0.0001 and ($packet->yaw % 360) === $this->yaw and ($packet->pitch % 360) === $this->pitch) break;
			...
			case ProtocolInfo::ADVENTURE_SETTINGS_PACKET:
				if(($packet->isFlying !== $this->isFlying()) or (!$this->allowFlight && ($packet->flags >> 9) & 0x01 === 1) || (!$this->isSpectator() && ($packet->flags >> 7) & 0x01 === 1)){
					$this->server->getPluginManager()->callEvent($ev = new PlayerToggleFlightEvent($this, $packet->isFlying));
					if($packet->isFlying and !$this->allowFlight) $ev->setCancelled();
			...
			case ProtocolInfo::MOB_EQUIPMENT_PACKET:
				if(!$this->spawned or !$this->isAlive()) break;

                /** @var MobEquipmentPacket $packet */
			    if($packet->windowId === 119){ // Offhand
			        if($this->offhandInventory !== null) $this->getTransactionQueue()->addTransaction(new BaseTransaction($this->offhandInventory, 0, $packet->item));
			        break;
                }
			...
			case ProtocolInfo::USE_ITEM_PACKET:
				...
				if($packet->face >= 0 and $packet->face <= 5){ //Use Block, place
					...
					if(!$this->canInteract($blockVector->add(0.5, 0.5, 0.5), 13) or $this->isSpectator()){
						...
						if($this->level->useItemOn($blockVector, $item, $packet->face, $packet->fx, $packet->fy, $packet->fz, $this)) break;
					...
					$this->level->sendBlocks([$this], array_merge($this->level->getBlock($blockVector)->getAllSides(), $this->level->getBlock($blockVector)->getSide($packet->face)->getAllSides()), UpdateBlockPacket::FLAG_ALL_PRIORITY);
					...
					$ev = new PlayerInteractEvent($this, $item, (new Vector3($packet->x / 32768, $packet->y / 32768, $packet->z / 32768))->normalize(), $packet->face, PlayerInteractEvent::RIGHT_CLICK_AIR);
					...
					switch($item->getId()){
                        case Item::FISHING_ROD:
                            ...
                            if(!$ev->isCancelled()){
                                if(!$this->isFishing()) $this->startFishing();
						...
						break;
						case Item::EYE_OF_ENDER:
							$entity = Entity::createEntity("EyeOfEnder", $this->getLevel(), $nbt, $this);
						break;
					}
			...
			case ProtocolInfo::PLAYER_ACTION_PACKET:
				if(!$this->spawned or (!$this->isAlive() and $packet->action !== PlayerActionPacket::ACTION_SPAWN_SAME_DIMENSION and $packet->action !== PlayerActionPacket::ACTION_SPAWN_OVERWORLD and $packet->action !== PlayerActionPacket::ACTION_SPAWN_NETHER)) break;
				...
				switch($packet->action){
					case PlayerActionPacket::ACTION_START_BREAK:
						...
						if($this->level->checkSpawnProtection($this, $target)) $ev->setCancelled();
						...
						if(!$ev->isCancelled()){
							...
							if(!$this->isCreative()){
								...
								if($breakTime > 0) $this->level->broadcastLevelEvent($pos, LevelEventPacket::EVENT_BLOCK_START_BREAK, (int) (65535 / $breakTime));
							}
						...
						if($this->startAction > -1 and $this->getDataFlag(self::DATA_FLAGS, self::DATA_FLAG_ACTION)){
							if($this->inventory->getItemInHand()->getId() === Item::BOW){
								...
								$nbt = new CompoundTag("", [
									...
									"Infinity" => new StringTag("Infinity", "false"),
									"Potion" => new ShortTag("Potion", $arrow->getDamage())
								]);
								if($bow->hasEnchantment(22)) $nbt->Infinity = new StringTag("Infinity", "true");
								$diff = ($this->server->getTick() - $this->startAction);
								$p = $diff / 20;
								$f = min((($p ** 2) + $p * 2) / 3, 1) * 2;
								$arrowent = Entity::createEntity("Arrow", $this->getLevel(), $nbt, $this, $f == 2 ? true : false);
								$arrowent->setMotion($arrowent->getMotion()->multiply(2));
								if($bow->hasEnchantment(21)) $arrowent->setOnFire(15);
								$ev = new EntityShootBowEvent($this, $bow, $arrowent, $f);
								...
								if($ev->isCancelled()){
									...
									if((!$bow->hasEnchantment(22) or $arrow->getDamage() !== 0) and $this->isSurvival()) $this->inventory->removeItem($arrow);
									if($this->isSurvival()){
					...
					case PlayerActionPacket::ACTION_SPAWN_OVERWORLD:
						if($this->isAlive() or !$this->isOnline()) break;
					...
					case PlayerActionPacket::ACTION_START_GLIDE:
						...
						$this->server->getPluginManager()->callEvent($ev);
						if($ev->isCancelled()) $this->sendData($this);
						else $this->setGliding(true);
					break 2;
					case PlayerActionPacket::ACTION_STOP_GLIDE:
						$ev = new PlayerToggleGlideEvent($this, false);
						$this->server->getPluginManager()->callEvent($ev);
						if($ev->isCancelled()) $this->sendData($this);
						else $this->setGliding(false);
					break 2;
			...
			case ProtocolInfo::REMOVE_BLOCK_PACKET:
				...
				if($this->canInteract($pos->add(0.5, 0.5, 0.5), $this->isCreative() ? 13 : 7) and $this->level->useBreakOn($pos, $item, $this, $this->server->destroyBlockParticle)){
					if($this->isSurvival()){
						if(!$item->equals($oldItem) or $item->getCount() !== $oldItem->getCount()){
							$this->inventory->setItemInHand($item);
							$this->inventory->sendHeldItem($this);
				...
				/** @var Block[] $blocks */
				$blocks = $this->level->getBlock($pos)->getAllSides();
				$blocks[] = $this->level->getBlock($pos);
			...
			case ProtocolInfo::INTERACT_PACKET:
				...
				if($target instanceof Player and !$this->server->getConfigBoolean("pvp", true)) $cancelled = true;
				...
				if($target instanceof Boat or ($target instanceof Minecart and $target->getType() == Minecart::TYPE_NORMAL)){
					if($packet->action === InteractPacket::ACTION_RIGHT_CLICK){
						...
						if($this->linkedEntity === $target) $target->setLinked(0, $this);
						$target->close();
				...
				if($packet->action === InteractPacket::ACTION_RIGHT_CLICK){
					if($target->getName() === "Cow" and $this->getInventory()->getItemInHand()->getId() === 325 and $this->getInventory()->getItemInHand()->getDamage() === 0){
						$item = $this->getInventory()->getItemInHand();
						$item->setCount($item->getCount() - 1);
						$this->getInventory()->setItemInHand($item);
						$this->getInventory()->addItem(Item::get(325, 1, 1));
						$this->getLevel()->addSound(new MilkSound($this->asVector3()));
					}elseif($target->getName() === "Sheep" and $this->getInventory()->getItemInHand()->getId() === 359){
						if($target->isSheared()) return;
						$target->setDataFlag(Entity::DATA_FLAGS, Entity::DATA_FLAG_SHEARED, true);
						$target->getLevel()->dropItem(new Vector3($target->getX(), $target->getY() + 1, $target->getZ()), Item::get(35, $target->getColor(), mt_rand(1, 3)));
						$item = $this->getInventory()->getItemInHand();
						$this->getLevel()->addSound(new ShearSound($this->asVector3()));
						if($item->getDamage() >= 238) return $this->getInventory()->setItemInHand(Item::get(0, 0, 0));
						$item->setDamage($item->getDamage() + 1);
						$this->getInventory()->setItemInHand($item);
					}elseif($target->getName() === "Creeper" and $this->getInventory()->getItemInHand()->getId() === 259){
						$item = $this->getInventory()->getItemInHand();
						if($item->getDamage() >= 65) $this->getInventory()->setItemInHand(Item::get(0, 0, 0));
						else{
							$item->setDamage($item->getDamage() + 1);
							$this->getInventory()->setItemInHand($item);
						}
						$expl = new Explosion(new Position($target->getX(), $target->getY(), $target->getZ(), $target->getLevel()), 6);
						$expl->explodeB();
						$expl->explodeA();
						$target->close();
						$target->kill();
					...
					break;
				}elseif($packet->action === InteractPacket::ACTION_MOUSEOVER) break;

				if($target instanceof Entity and $this->getGamemode() !== Player::VIEW and $this->isAlive() and $target->isAlive()){
					if($target instanceof DroppedItem or $target instanceof Arrow) break;
					...
					if(!$this->canInteract($target, 8) or $this->isSpectator()){
						...
						if(($target->getGamemode() & 0x01) > 0) break;
						elseif(!$this->server->getConfigBoolean("pvp") or $this->server->getDifficulty() === 0) $cancelled = true;
					}
					...
					if(!$this->isSprinting() and !$this->isFlying() and $this->fallDistance > 0 and !$this->hasEffect(Effect::BLINDNESS) and !$this->isInsideOfWater()) $ev->setDamage($ev->getFinalDamage() / 2, EntityDamageEvent::MODIFIER_CRITICAL);
					...
					if($this->isSurvival()){
						if($item->isTool()){
							if($item->useOn($target) and $item->getDamage() >= $item->getMaxDurability()) $this->inventory->setItemInHand(Item::get(Item::AIR, 0, 1));
			...
			case ProtocolInfo::COMMAND_STEP_PACKET:
				...
				if($packet->inputJson !== null){
					if(is_countable($packet->inputJson)){
					    if(sizeof($packet->inputJson) > 15 and $this->close("", "", false)) return $this->server->getNetwork()->blockAddress($this->getAddress(), 1600);
					}

					foreach($packet->inputJson as $arg){ //command ordering will be an issue
						if(!is_object($arg)) $packet->command .= " " . $arg;
					}
				}
				$this->server->getPluginManager()->callEvent($ev = new PlayerCommandPreprocessEvent($this, "/" . $packet->command));
				if($ev->isCancelled()) break;
			...
			case ProtocolInfo::TEXT_PACKET:
				...
				if($packet->type === TextPacket::TYPE_CHAT){
					if(isset($packet->message{1000}) and $this->close('', '')){
						$this->server->getNetwork()->blockAddress($this->getAddress(), 3600);
						break;
					}
					...
					foreach(explode("\n", $packet->message) as $message){
						if(trim($message) != "" and strlen($message) <= 255 and --$this->messageCounter > 0){
							if(substr($message, 0, 2) === "./") $message = substr($message, 1);
			...
			case ProtocolInfo::CONTAINER_CLOSE_PACKET:
				if(!$this->spawned or $packet->windowid === 0) break;
				$this->craftingType = self::CRAFTING_SMALL;
			...
			case ProtocolInfo::CRAFTING_EVENT_PACKET:
				...
				if($this->craftingType === self::CRAFTING_ANVIL){
					...
					if($recipe === null){
						if($packet->output[0]->getId() > 0 && $packet->output[1] === 0) $anvilInventory->onRename($this, $packet->output[0]);
						elseif($packet->output[0]->getId() > 0 && $packet->output[1] > 0) $anvilInventory->process($this, $packet->output[0], $packet->output[1]);
					}
					break;
				...
				if(sizeof($packet->input) === 0){
					...
					if(!$packet->output[0]->equals($recipe->getResult())) $this->server->getLogger()->debug("Mismatched desktop recipe received from player " . $this->getName() . ", expected " . $recipe->getResult() . ", got " . $packet->output[0]);
					$recipe = null;
					...
					if($recipe instanceof ShapedRecipe){
						...
						if(sizeof($needed) > 0) $canCraft = false;
					}else $canCraft = false;
					...
					if(sizeof($extraItem) > 0 and !$this->isCreative()){ //Could not add all the items to our inventory (not enough space)
			...
			case ProtocolInfo::BLOCK_ENTITY_DATA_PACKET:
				...
				$pos = new Vector3($packet->x, $packet->y, $packet->z);
				if($pos->distanceSquared($this) > 10000 or $this->level->checkSpawnProtection($this, $pos)) break;
				...
				if($t instanceof Spawnable){
					...
					$nbt = $nbt->getData();
					if(!$t->updateCompoundTag($nbt, $this)) $t->spawnTo($this);
				}
				break;
			case ProtocolInfo::REQUEST_CHUNK_RADIUS_PACKET:
				if($packet->radius >= 60 and $this->close('', '')){
					$this->server->getNetwork()->blockAddress($this->getAddress(), 3600);
					break;
				}
			...
			case ProtocolInfo::ITEM_FRAME_DROP_ITEM_PACKET:
				...
				if($tile instanceof ItemFrame){
					...
					if(lcg_value() <= $tile->getItemDropChance()) $this->level->dropItem($tile->getBlock(), $tile->getItem());
					$tile->setItem(null);
	...
	public function kick(string $reason = "", bool $isAdmin = true, $quitMessage = null) : bool{
		...
		if(!$ev->isCancelled()){
			...
			$message = $reason;
			if($isAdmin){
				if(!$this->isBanned()) $message = "Kicked by admin." . ($reason !== "" ? " Reason: " . $reason : "");
			}elseif($reason === "") $message = "disconnectionScreen.noReason";
	...
	public function dropItem(Item $item){
		if(!$this->spawned or !$this->isAlive()) return;
		...
		$entity = $this->level->dropItem($this->add(0, 1.5, 0), $item, $this->getDirectionVector()->multiply(0.3), 40);

		$this->server->getPluginManager()->callEvent($ev = new PlayerDropItemEvent($this, $item, $entity));
		if($ev->isCancelled()){
			$entity->close();
			$this->getFloatingInventory()->removeItem($item);
			$this->getInventory()->addItem($item);
	...
	public function sendActionBar(string $title, string $subtitle = "", int $fadeIn = -1, int $stay = -1, int $fadeOut = -1){
		...
		if($subtitle !== "") $this->sendTitleText($subtitle, SetTitlePacket::TYPE_SUB_TITLE);
	...
	protected function respawn() : void{
		...
		$this->teleport(Position::fromObject($ev->getRespawnPosition()->add(0.5, 0, 0.5), $ev->getRespawnPosition()->getLevel()));
		...
		foreach($this->attributeMap->getAll() as $attr) $attr->resetToDefault();
	...
	public function sendTranslation($message, array $parameters = []){
		...
		$this->server->getPluginManager()->callEvent($ev = new PlayerTextPreSendEvent($this, $pk->message, PlayerTextPreSendEvent::TRANSLATED_MESSAGE));
		if(!$ev->isCancelled()){
			$this->dataPacket($pk);
			return true;
		}
	...
	public function sendPopup($message, $subtitle = ""){
		$this->server->getPluginManager()->callEvent($ev = new PlayerTextPreSendEvent($this, $message, PlayerTextPreSendEvent::POPUP));
		if(!$ev->isCancelled()){
			$pk = new TextPacket();
	...
	public function sendTip($message){
		$this->server->getPluginManager()->callEvent($ev = new PlayerTextPreSendEvent($this, $message, PlayerTextPreSendEvent::TIP));
		if(!$ev->isCancelled()){
			$pk = new TextPacket();
		...
		if($this->connected and !$this->closed){
			...
			if($this->isValid()){
			    foreach($this->usedChunks as $index => $d){
				    ...
				    foreach($this->level->getChunkEntities($chunkX, $chunkZ) as $entity) $entity->despawnFrom($this);
				    unset($this->usedChunks[$index]);
				}
			}
			$this->usedChunks = [];
			...
			if($this->constructed) parent::close();
			$this->spawned = false;
	...
	public function save(){
		...
		if($this->isValid()) $this->namedtag->Level = new StringTag("Level", $this->level->getFolderName());
		...
		foreach($this->achievements as $achievement => $status) $this->namedtag->Achievements[$achievement] = new ByteTag($achievement, $status === true ? 1 : 0);
		...
		if($this->username != "") $this->server->saveOfflinePlayerData($this->username, $this->namedtag);
	}
	...
	protected function callDeathEvent(){
		...
		switch($cause === null ? EntityDamageEvent::CAUSE_CUSTOM : $cause->getCause()){
			...
			case EntityDamageEvent::CAUSE_SUFFOCATION: $message = "death.attack.inWall"; break;

			case EntityDamageEvent::CAUSE_LAVA: $message = "death.attack.lava"; break;

			case EntityDamageEvent::CAUSE_FIRE: $message = "death.attack.onFire"; break;
			...
			case EntityDamageEvent::CAUSE_CONTACT:
				if($cause instanceof EntityDamageByBlockEvent){
					if($cause->getDamager()->getId() === Block::CACTUS) $message = "death.attack.cactus";
				}
				break;
		...
		if(!$ev->getKeepInventory()){
			foreach($ev->getDrops() as $item) $this->level->dropItem($this, $item);

			if($this->floatingInventory !== null) $this->floatingInventory->clearAll();		

			if($this->inventory !== null) $this->inventory->clearAll();

			if($this->offhandInventory !== null) $this->offhandInventory->clearAll();
		}

		if($this->server->expEnabled and !$ev->getKeepExperience()){
			$this->getLevel()->spawnXPOrb($this->add(0, 0.2, 0), min(91, $this->getTotalXp()));
	...
	public function addHealth($amount){
		return $this->setHealth($this->getHealth() + $amount);
	}
	/**
	 * @param float             $damage
	...
	public function sendPosition(Vector3 $pos, float $yaw = null, float $pitch = null, int $mode = MovePlayerPacket::MODE_NORMAL, array $targets = null){
		...
		if($targets !== null){
			if(in_array($this, $targets, true)) $this->ySize = 0;
	...
	public function teleport(Vector3 $pos, float $yaw = null, float $pitch = null) : bool{
		if(parent::teleport($pos, $yaw, $pitch)){
			...
			$this->resetFallDistance();
			$this->nextChunkOrderRun = 0;
			if($this->spawnChunkLoadCount !== -1) $this->spawnChunkLoadCount = 0;
			$this->newPosition = null;
			$this->stopSleep();
	...
	public function addWindow(Inventory $inventory, $forceId = null) : int{
		if(($id = $this->getWindowId($inventory)) !== -1) return $id;
	...
	public static function isValidUserName(string $name) : bool{
		...
		return $lname !== "rcon" and $lname !== "console" and $lname !== "steve" and strlen($name) >= 1 and strlen($name) <= 16 and preg_match("/[^A-Za-z0-9_]/", $name) === 0;
	}
	public function isValidDeviceOS($os){
		if($os === null or !is_numeric($os)) return false;
		return true;
	}
