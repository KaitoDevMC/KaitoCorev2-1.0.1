...
/**
 * The class that manages everythingmotd
 */
class Server{
	...
	public function setAutoSave($value){
		...
		foreach($this->getLevels() as $level) $level->setAutoSave($this->autoSave);
	}
	...
	public function getTicksPerSecondAverage(){
		return round(array_sum($this->tickAverage) / sizeof($this->tickAverage), 2);
	}
	...
	public function getTickUsageAverage(){
		return round((array_sum($this->useAverage) / sizeof($this->useAverage)) * 100, 2);
	}
	...
	public function getOfflinePlayer(string $name){
		...
		if($result === null) $result = new OfflinePlayer($this, strtolower($name));
	...
	public function getOfflinePlayerData(string $name){
		...
		if($this->shouldSavePlayerData()){
			...
			}else $this->logger->notice($this->getLanguage()->translateString("pocketmine.data.playerNotFound", [$name]));
		}
		...
		$nbt = new CompoundTag("", [
			...
			new ListTag("Pos", [
				new DoubleTag(0, $this->getDefaultLevel()->getSafeSpawn()->x),
				new DoubleTag(1, $this->getDefaultLevel()->getSafeSpawn()->y),
				new DoubleTag(2, $this->getDefaultLevel()->getSafeSpawn()->z)
			]),
	...
	public function getPlayer(string $name){
		...
		foreach($this->getOnlinePlayers() as $player){
			if(stripos($player->getName(), strtolower($name)) === 0){
				$curDelta = strlen($player->getName()) - strlen($name);
				if($curDelta < PHP_INT_MAX) $found = $player;
				if($curDelta === 0) break;
			}
		}
	...
	public function matchPlayer($partialName){
		$matchedPlayers = [];
		foreach($this->getOnlinePlayers() as $player){
			if(strtolower($player->getName()) === strtolower($partialName)){
				$matchedPlayers = [$player];
				break;
			}elseif(stripos($player->getName(), strtolower($partialName)) !== false) $matchedPlayers[] = $player;
		}
	...
	public function getLevelByName($name){
		foreach($this->getLevels() as $level){
			if($level->getFolderName() === $name) return $level;
		}
	...
	public function unloadLevel(Level $level, $forceUnload = false){
		if($level === $this->getDefaultLevel() and !$forceUnload) throw new \InvalidStateException('The default world cannot be unloaded while running, please switch worlds.');
	...
	public function loadLevel($name){
		if(trim($name) === "") throw new LevelException('Invalid empty level name');
	...
	public function generateLevel($name, $seed = null, $generator = null, $options = []){
		if(trim($name) === "" or $this->isLevelGenerated($name)) return false;

		$seed = $seed ?? random_int(INT32_MIN, INT32_MAX);

		if(!isset($options["preset"])) $options["preset"] = $this->getConfigString("generator-settings", "");

		if(!($generator !== null and class_exists($generator, true) and is_subclass_of($generator, Generator::class))) $generator = Generator::getGenerator($this->getLevelType());
		...
		if(($provider = LevelProviderManager::getProviderByName($providerName = $this->getProperty("level-settings.default-format", "pmanvil"))) === null){
			...
			if($provider === null) throw new \InvalidStateException("Default level provider has not been registered");
		}
	...
	public function findEntity(int $entityId, Level $expectedLevel = null){
        foreach($this->levels as $level){
			...
			if(($entity = $level->getEntity($entityId)) instanceof Entity) return $entity;
		}
	...
	public function getConfigString($variable, $defaultValue = ""){
		$v = getopt("", ["$variable::"]);
		if(isset($v[$variable])) return (string) $v[$variable];
	...
	public function getProperty($variable, $defaultValue = null){
		if(!array_key_exists($variable, $this->propertyCache)){
			$v = getopt("", ["$variable::"]);
			if(isset($v[$variable])) $this->propertyCache[$variable] = $v[$variable];
			else $this->propertyCache[$variable] = $this->config->getNested($variable);
		}
	...
	public function getConfigInt($variable, $defaultValue = 0){
		$v = getopt("", ["$variable::"]);
		if(isset($v[$variable])) return (int) $v[$variable];
	...
	public function getConfigBoolean($variable, $defaultValue = false){
		$v = getopt("", ["$variable::"]);
		if(isset($v[$variable])) $value = $v[$variable];
		else $value = $this->properties->exists($variable) ? $this->properties->get($variable) : $defaultValue;
	...
	public function getPluginCommand($name){
		if(($command = $this->commandMap->getCommand($name)) instanceof PluginIdentifiableCommand) return $command;
		else return null;
	}
	...
	public function addOp($name){
		$this->operators->set(strtolower($name), true);
	 	...
	 	if(($player = $this->getPlayerExact($name)) !== null) $player->recalculatePermissions();
		$this->operators->save();
	}
	...
	public function removeOp($name){
		foreach($this->operators->getAll() as $opName => $dummyValue){
			if(strtolower($name) === strtolower($opName)) $this->operators->remove($opName);
		}
	...
	public function getCommandAliases(){
		...
		if(is_array($section)){
			foreach($section as $key => $value){
				$commands = [];
				if(is_array($value)) $commands = $value;
				else $commands[] = (string) $value;
	...
	public static function getInstance() : Server{
		if(self::$instance === null) throw new \RuntimeException("Attempt to retrieve Server instance outside server thread");
		return self::$instance;
	}

	public static function microSleep(int $microseconds){
		if(self::$sleeper === null) self::$sleeper = new \Threaded();
	...
	public function loadAdvancedConfig(){
		...
		$this->mobSpawnEnabled = $this->getAdvancedProperty("mob.allow-spawn", false);
	...
	public function getBuild(){
		$version = new VersionString($this->getPocketMineVersion()->getBuild());
		return $version;
	}

	public function getGameVersion(){
		$version = new VersionString($this->getPocketMineVersion()->getRelease());
		return $version;
	}
	...
	public function onLoad(){
		try{
			if(!file_exists($this->dataPath . "worlds/")) mkdir($this->dataPath . "worlds/", 0777);

			if(!file_exists($this->dataPath . "players/")) mkdir($this->dataPath . "players/", 0777);

			if(!file_exists($this->pluginPath)) mkdir($this->pluginPath, 0777);

			if(!file_exists($this->dataPath . "crashdumps/")) mkdir($this->dataPath . "crashdumps/", 0777);

			$this->dataPath = realpath($this->dataPath) . DIRECTORY_SEPARATOR;
			$this->pluginPath = realpath($this->pluginPath) . DIRECTORY_SEPARATOR;
			...
			if(!file_exists($this->dataPath . "pocketmine.yml")){
				if(file_exists($this->dataPath . "lang.txt")){
					...
					if(file_exists(\pocketmine\PATH . "src/pocketmine/resources/pocketmine_$wizardLang.yml")) $content = file_get_contents($file = \pocketmine\PATH . "src/pocketmine/resources/pocketmine_$wizardLang.yml");
					else $content = file_get_contents($file = \pocketmine\PATH . "src/pocketmine/resources/pocketmine_rus.yml");
				}else $content = file_get_contents($file = \pocketmine\PATH . "src/pocketmine/resources/pocketmine_rus.yml");
				@file_put_contents($this->dataPath . "pocketmine.yml", $content);
			}
			if(file_exists($this->dataPath . "lang.txt")) unlink($this->dataPath . "lang.txt");
			$this->config = new Config($configPath = $this->dataPath . "pocketmine.yml", Config::YAML, []);

			if($this->defaultLang != "unknown" and $this->getProperty("settings.language", "rus") != $this->defaultLang){
				@file_put_contents($configPath, str_replace('language: "' . $this->getProperty("settings.language", "rus") . '"', 'language: "' . $this->defaultLang . '"', file_get_contents($configPath)));
				$this->config->reload();
				unset($this->propertyCache["settings.language"]);
			}

			$lang = $this->getProperty("settings.language", BaseLang::FALLBACK_LANGUAGE);
			if(file_exists(\pocketmine\PATH . "src/pocketmine/resources/lite_$lang.yml")) $content = file_get_contents($file = \pocketmine\PATH . "src/pocketmine/resources/lite_$lang.yml");
			else $content = file_get_contents($file = \pocketmine\PATH . "src/pocketmine/resources/lite_rus.yml");

			if(!file_exists($this->dataPath . "lite.yml")) @file_put_contents($this->dataPath . "lite.yml", $content);
			...
			if(!extension_loaded("openssl")) $this->setConfigBool("online-mode", false);
			...
			if(($poolSize = $this->getProperty("settings.async-workers", "auto")) === "auto"){
				...
				if($processors > 0) $poolSize = max(1, $processors);
			}else $poolSize = max(1, (int) $poolSize);

			ServerScheduler::$WORKERS = $poolSize;

			if($this->getProperty("network.batch-threshold", 256) >= 0) Network::$BATCH_THRESHOLD = (int) $this->getProperty("network.batch-threshold", 256);
			else Network::$BATCH_THRESHOLD = -1;
			...
			$sleeper = new SleeperNotifier();
			$this->console = new CommandReader($sleeper);
			...
			if($this->getConfigBoolean("enable-rcon", false)){
				try{
					...
					$this->getLogger()->critical("Не удалось запустить RCON: " . $e->getMessage());
				}
			}
			...
			if(((int) ini_get('zend.assertions')) !== -1) $this->logger->warning('Debugging assertions are enabled, this may impact on performance. To disable them, set `zend.assertions = -1` in php.ini.');

			ini_set('assert.exception', '1');

			if($this->logger instanceof MainLogger) $this->logger->setLogDebug(\pocketmine\DEBUG > 1);

			if(\pocketmine\DEBUG >= 0) @cli_set_process_title($this->getName() . " " . $this->getPocketMineVersion());
			...
			foreach((array) $this->getProperty("worlds", []) as $name => $options){
				...
				elseif(!is_array($options)) continue;
				if(!$this->loadLevel($name)){
					$seed = $options["seed"] ?? time();
					if(is_string($seed) and !is_numeric($seed)) $seed = Utils::javaStringHash($seed);
					elseif(!is_int($seed)) $seed = (int) $seed;
			...
			if($this->getDefaultLevel() === null){
				...
				if(!$this->loadLevel($default)){
					...
					if(!is_numeric($seed) or bccomp($seed, "9223372036854775807") > 0) $seed = Utils::javaStringHash($seed);
					else $seed = (int) $seed;
			...
			if($this->properties->hasChanged()) $this->properties->save();
			...
			if($this->netherEnabled){
				if(!$this->loadLevel($this->netherName)) $this->generateLevel($this->netherName, time(), Generator::getGenerator("nether"));
				$this->netherLevel = $this->getLevelByName($this->netherName);
			}

			if($this->enderEnabled){
				if(!$this->loadLevel($this->enderName)) $this->generateLevel($this->enderName, time(), Generator::getGenerator("ender"));
				$this->enderLevel = $this->getLevelByName($this->enderName);
			}

			if($this->getProperty("ticks-per.autosave", 6000) > 0) $this->autoSaveTicks = (int) $this->getProperty("ticks-per.autosave", 6000);
	...
	public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $dataPath, $pluginPath, $defaultLang = 'unknown'){
		if(self::$instance !== null) throw new \InvalidStateException('Only one server instance can exist at once');
		self::$instance = $this;
	...
	public function broadcastMessage($message, $recipients = null) : int{
		if(!is_array($recipients)) return $this->broadcast($message, self::BROADCAST_CHANNEL_USERS);

		/** @var Player[] $recipients */
		foreach($recipients as $recipient) $recipient->sendMessage($message);
	...
	public function broadcastTip(string $tip, $recipients = null) : int{
		if(!is_array($recipients)){
			...
			foreach($this->pluginManager->getPermissionSubscriptions(self::BROADCAST_CHANNEL_USERS) as $permissible){
				if($permissible instanceof Player and $permissible->hasPermission(self::BROADCAST_CHANNEL_USERS)) $recipients[spl_object_hash($permissible)] = $permissible; // do not send messages directly, or some might be repeated
			}
		}

		/** @var Player[] $recipients */
		foreach($recipients as $recipient) $recipient->sendTip($tip);
	...
	public function broadcastPopup(string $popup, $recipients = null) : int{
		...
		/** @var Player[] $recipients */
		foreach($recipients as $recipient) $recipient->sendPopup($popup);
	...
	public function broadcastTitle(string $title, string $subtitle = "", int $fadeIn = -1, int $stay = -1, int $fadeOut = -1, $recipients = null){
		...
		/** @var Player[] $recipients */
		foreach($recipients as $recipient) $recipient->addTitle($title, $subtitle, $fadeIn, $stay, $fadeOut);
	...
	public function broadcast($message, string $permissions) : int{
		...
		foreach(explode(";", $permissions) as $permission){
			foreach($this->pluginManager->getPermissionSubscriptions($permission) as $permissible){
				if($permissible instanceof CommandSender and $permissible->hasPermission($permission)) $recipients[spl_object_hash($permissible)] = $permissible; // do not send messages directly, or some might be repeated
			}
		}
	...
	public function batchPackets(array $players, array $packets, bool $forceSync = false, bool $immediate = false){
		if(sizeof($packets) === 0) throw new \InvalidArgumentException('Cannot send empty batch');
		...
		if(sizeof($targets) > 0){
			...
			foreach($packets as $p) $pk->addPacket($p);
			...
			}else $this->broadcastPacketsCallback($pk, $targets, $immediate);
		}
	...
	public function broadcastPacketsCallback(BatchPacket $pk, array $players, bool $immediate = false){
		...
		foreach($players as $i) $i->dataPacket($pk, false, $immediate);
	}
	...
	public function enablePlugins(int $type){
		foreach($this->pluginManager->getPlugins() as $plugin){
			if(!$plugin->isEnabled() and $plugin->getDescription()->getOrder() === $type) $this->pluginManager->enablePlugin($plugin);
		}
	...
	public function reload(){
		...
		foreach($this->levels as $level) $level->save();
		...
		if($this->getConfigBoolean("hardcore", false) and $this->getDifficulty() < 3) $this->setConfigInt("difficulty", 3);
		...
		foreach($this->getIPBans()->getEntries() as $entry) $this->getNetwork()->blockAddress($entry->getName(), -1);

		$this->pluginManager->registerInterface(PharPluginLoader::class);
		if($this->folderpluginloader) $this->pluginManager->registerInterface(FolderPluginLoader::class);
		$this->pluginManager->registerInterface(ScriptPluginLoader::class);
		$this->pluginManager->loadPlugins($this->pluginPath);
		$this->enablePlugins(PluginLoadOrder::STARTUP);
		$this->enablePlugins(PluginLoadOrder::POSTWORLD);
	...
	public function shutdown(bool $restart = false, string $msg = ""){
		...
		if($msg != "") $this->propertyCache["settings.shutdown-message"] = $msg;
	}

	public function forceShutdown(){
		if($this->hasStopped) return;

		if($this->doTitleTick) echo "\x1b]0;\x07";

		try{
			if(!$this->isRunning()) $this->sendUsage(SendUsageTask::TYPE_CLOSE);

			$this->hasStopped = true;

			$this->shutdown();
			if($this->rcon instanceof RCON) $this->rcon->stop();

			if($this->getProperty("network.upnp-forwarding", false)) UPnP::RemovePortForward($this->getPort());

			if($this->pluginManager instanceof PluginManager) $this->pluginManager->disablePlugins();

			foreach($this->getLevels() as $level) $this->unloadLevel($level, true);

			HandlerList::unregisterAll();

			if($this->scheduler instanceof ServerScheduler) $this->scheduler->shutdown();

			if($this->properties !== null and $this->properties->hasChanged()) $this->properties->save();
			...
			$this->logger->logException($e);
			$this->logger->emergency('Краш, Фикси ошибку! Отключение плагина...');
			@Utils::kill(getmypid());
		}
	...
	public function start(){
		if($this->getConfigBoolean("enable-query", true)) $this->queryHandler = new QueryHandler();
		...
		if(!file_exists($this->getPluginPath() . DIRECTORY_SEPARATOR . "LiteCore - Vanilla")) @mkdir($this->getPluginPath() . DIRECTORY_SEPARATOR . "LiteCore - Vanilla");
	...
	public function handleSignal($signo){
		if($signo === SIGTERM or $signo === SIGINT or $signo === SIGHUP) $this->shutdown();
	}
	...
	public function exceptionHandler(\Throwable $e, $trace = null){
		...
		if($trace === null) $trace = $e->getTrace();
	...
	public function crashDump(){
		...
		if($this->sendUsageTicker > 0) $this->sendUsage(SendUsageTask::TYPE_CLOSE);
		...
		if(file_exists($stamp) and !($report = (filemtime($stamp) + 120 < time()))) $this->logger->debug("Not sending crashdump due to last crash less than 120 seconds ago");
		...
		$spacing = ((int) \pocketmine\START_TIME) - time() + 30;
		if($spacing > 0){
			echo '--- Жди 30 секунд чтобы сервер сам включился (ты можешь удалить этот процесс CTRL + C) ---' . PHP_EOL;
			sleep($spacing);
		}
		@Utils::kill(getmypid());
		exit(1);
	...
	private function tickProcessor(){
		...
		while($this->isRunning){
			...
			if(($tickTime - $this->nextTick) < -0.025) return;
			...
			foreach($this->players as $player) $player->checkNetwork();

			if(($this->tickCounter % 20) === 0){
				if($this->doTitleTick) $this->titleTick();
				$this->currentTPS = 20;
				$this->currentUse = 0;

				if(($this->dserverConfig["enable"] and $this->dserverConfig["queryTickUpdate"]) or !$this->dserverConfig["enable"]) $this->getPluginManager()->callEvent($this->queryRegenerateTask = new QueryRegenerateEvent($this));

				$this->network->updateName();
				$this->network->resetStatistics();

	            if($this->dserverConfig["enable"] and $this->dserverConfig["motdPlayers"]) $this->getNetwork()->setName($this->getNetwork()->getName().'['.$this->getDServerOnlinePlayers().'/'.$this->getDServerMaxPlayers().']');
			}
			...
			if(($this->tickCounter % 100) === 0){
				foreach($this->levels as $level) $level->clearCache();
			}

			if($this->dispatchSignals and $this->tickCounter % 5 === 0) pcntl_signal_dispatch();
	...
	public function onPlayerLogin(Player $player){
		if($this->sendUsageTicker > 0) $this->uniquePlayers[$player->getRawUniqueId()] = $player->getRawUniqueId();
	...
	public function sendFullPlayerListData(Player $p){
		$pk = new PlayerListPacket();
		$pk->type = PlayerListPacket::TYPE_ADD;
		foreach($this->playerList as $player) $pk->entries[] = [$player->getUniqueId(), $player->getId(), $player->getDisplayName(), $player->getSkinId(), $player->getSkinData()];
	...
	private function checkTickUpdates(int $currentTick, float $tickTime) : void{
		...
		foreach($this->levels as $k => $level){
			if(!isset($this->levels[$k])) continue;
			$levelTime = microtime(true);
			$level->doTick($currentTick);
			$tickMs = (microtime(true) - $levelTime) * 1000;
			$level->tickRateTime = $tickMs;
			if($tickMs >= 50) $this->getLogger()->debug(sprintf("World \"%s\" took too long to tick: %gms (%g ticks)", $level->getName(), $tickMs, round($tickMs / 50, 2)));
		}
	}
	...
	public function doAutoSave(){
		if($this->getAutoSave()){
			...
			foreach($this->players as $index => $player){
				if($player->spawned) $player->save();
				elseif(!$player->isConnected()) $this->removePlayer($player);
			}

			foreach($this->getLevels() as $level) $level->save(false);
	...
	private function titleTick(){
		...
		echo "\x1b]0;" . $this->getName() . $this->getFormattedVersion("-") .
			" | Онлайн сервера " . count($this->players) . "/" . $this->getMaxPlayers() .
			" | Память " . round(($u[0] / 1024) / 1024, 2) . "/" . round((Utils::getRealMemoryUsage()[0] / 1024) / 1024, 2) . "/" . round(($u[1] / 1024) / 1024, 2) . "/" . round(($u[2] / 1024) / 1024, 2) . " MB @ " . Utils::getThreadCount() . " потоков" .
			" | U " . round($this->network->getUpload() / 1024, 2) .
			" D " . round($this->network->getDownload() / 1024, 2) .
			" kB/s | ТПС " . $this->getTicksPerSecondAverage() .
	...
	public function handlePacket(AdvancedSourceInterface $interface, string $address, int $port, string $payload){
		...
		try{
			if(strlen($payload) > 2 and substr($payload, 0, 2) === "\xfe\xfd" and $this->queryHandler instanceof QueryHandler) $this->queryHandler->handle($interface, $address, $port, $payload);
			else $this->logger->debug("Unhandled raw packet from $address $port: " . base64_encode($payload));
		}catch(\Throwable $e){
			if($this->logger instanceof MainLogger) $this->logger->logException($e);
	...
	public function getAdvancedProperty($variable, $defaultValue = null, Config $cfg = null){
		...
		if($cfg->exists($base)) $base = $cfg->get($base);
		else return $defaultValue;

		while(sizeof($vars) > 0){
			$baseKey = array_shift($vars);
			if(is_array($base) and isset($base[$baseKey])) $base = $base[$baseKey];
			else return $defaultValue;
		}
