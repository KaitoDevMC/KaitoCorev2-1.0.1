...
class QueryRegenerateEvent extends ServerEvent {
	...
	public function __construct(Server $server){
		...
		foreach($server->getOnlinePlayers() as $player){
			if($player->isOnline()) $this->players[] = $player;
		}
		...
		// Slots dinámicos: 0/1 - 1/2 - 2/3 - 3/4 ...
	...
	public function getLongQuery(){
		if($this->longQueryCache !== null) return $this->longQueryCache;

		$plist = $this->server_engine;
		if(sizeof($this->plugins) > 0 and $this->listPlugins){
			$plist .= ":";
