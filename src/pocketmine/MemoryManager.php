...
class MemoryManager {
	...
	private function init(){
		...
		if($hardLimit <= 0) ini_set("memory_limit", '-1');
		else ini_set("memory_limit", $hardLimit . "M");
	...
	public function trigger($memory, $limit, $global = false, $triggerCount = 0){
		...
		$cycles = 0;
		if($this->garbageCollectionTrigger) $cycles = $this->triggerGarbageCollector();
	...
	public function check(){
		...
		if(($this->memoryLimit > 0 or $this->globalMemoryLimit > 0) and ++$this->checkTicker >= $this->checkRate){
			...
			$trigger = false;
			if($this->memoryLimit > 0 and $memory[0] > $this->memoryLimit) $trigger = 0;
			elseif($this->globalMemoryLimit > 0 and $memory[1] > $this->globalMemoryLimit) $trigger = 1;
	...
	public function addObjectWatcher($object){
		if(!is_object($object)) throw new \InvalidArgumentException("Not an object!");


		$identifier = spl_object_hash($object) . ":" . get_class($object);

		if(isset($this->leakInfo[$identifier])) return $this->leakInfo["id"];
	...
	public function isObjectAlive($id){
		if(isset($this->leakWatch[$id])) return $this->leakWatch[$id]->valid();
	...
	public function removeObjectWatch($id){
		if(!isset($this->leakWatch[$id])) return;
	...
	public function doObjectCleanup(){
		foreach($this->leakWatch as $id => $w){
			if(!$w->valid()) $this->removeObjectWatch($id);
		}
	}
	...
	public function getObjectInformation($id, $includeObject = false){
		if(!isset($this->leakWatch[$id])) return null;
	...
	public function dumpServerMemory($outputFolder, $maxNesting, $maxStringSize){
		...
		do{
			...
			foreach($objects as $hash => $object){
				...
				if(($parent = $reflection->getParentClass()) !== false) $info["parent"] = $parent->getName();

				if(count($reflection->getInterfaceNames()) > 0) $info["implements"] = implode(", ", $reflection->getInterfaceNames());
				...
				if(!isset($staticProperties[$className])){
					...
					foreach($reflection->getProperties() as $property){
						if(!$property->isStatic() or $property->getDeclaringClass()->getName() !== $className) continue;
