...
class Furnace extends Spawnable implements InventoryHolder, Container, Nameable {
	...
	public function __construct(Level $level, CompoundTag $nbt){
		if(!isset($nbt->BurnTime) or $nbt["BurnTime"] < 0) $nbt->BurnTime = new ShortTag("BurnTime", 0);
		if(!isset($nbt->CookTime) or !($nbt->CookTime instanceof ShortTag) or $nbt["CookTime"] < 0 or ($nbt["BurnTime"] === 0 and $nbt["CookTime"] > 0)) $nbt->CookTime = new ShortTag("CookTime", 0);
		if(!isset($nbt->MaxTime) or !($nbt->MaxTime instanceof ShortTag)){
			$nbt->MaxTime = new ShortTag("BurnTime", $nbt["BurnTime"]);
			unset($nbt->BurnTicks);
		}

		if(!isset($nbt->BurnTicks)) $nbt->BurnTicks = new ShortTag("BurnTicks", 0);
		...
		if($this->namedtag["BurnTime"] > 0) $this->scheduleUpdate();
	}
	...
	protected function getSlotIndex($index){
		foreach($this->namedtag->Items as $i => $slot){
			if((int) $slot["Slot"] === $index) return (int) $i;
		}
	...
	public function getItem($index){
		...
		if($i < 0) return Item::get(Item::AIR, 0, 0);
	...
	protected function checkFuel(Item $fuel){
		...
		if($this->getBlock()->getId() === Block::FURNACE) $this->getLevel()->setBlock($this, Block::get(Item::BURNING_FURNACE, $this->getBlock()->getDamage()), true);
	...
	public function onUpdate(){
		...
		if($this->namedtag["BurnTime"] <= 0 and $canSmelt and $fuel->getFuelTime() !== null and $fuel->getCount() > 0) $this->checkFuel($fuel);
		...
		if($this->namedtag["BurnTime"] > 0){
			...
			if($this->getBlock()->getId() === Block::BURNING_FURNACE) $this->getLevel()->setBlock($this, Block::get(Item::FURNACE, $this->getBlock()->getDamage()), true);
			$this->namedtag->BurnTime = new ShortTag("BurnTime", 0);
			$this->namedtag->CookTime = new ShortTag("CookTime", 0);
			$this->namedtag->BurnTicks = new ShortTag("BurnTicks", 0);
		}
