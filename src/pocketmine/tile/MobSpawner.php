...
class MobSpawner extends Spawnable {
	...
	public function __construct(Level $level, CompoundTag $nbt){
		if(!isset($nbt->EntityId) or !($nbt->EntityId instanceof IntTag)) $nbt->EntityId = new IntTag("EntityId", 0);
		if(!isset($nbt->SpawnCount) or !($nbt->SpawnCount instanceof IntTag)) $nbt->SpawnCount = new IntTag("SpawnCount", 4);
		if(!isset($nbt->SpawnRange) or !($nbt->SpawnRange instanceof IntTag)) $nbt->SpawnRange = new IntTag("SpawnRange", 4);
		if(!isset($nbt->MinSpawnDelay) or !($nbt->MinSpawnDelay instanceof IntTag)) $nbt->MinSpawnDelay = new IntTag("MinSpawnDelay", 200);
		if(!isset($nbt->MaxSpawnDelay) or !($nbt->MaxSpawnDelay instanceof IntTag)) $nbt->MaxSpawnDelay = new IntTag("MaxSpawnDelay", 799);
		if(!isset($nbt->Delay) or !($nbt->Delay instanceof IntTag)) $nbt->Delay = new IntTag("Delay", mt_rand($nbt->MinSpawnDelay->getValue(), $nbt->MaxSpawnDelay->getValue()));
	...
	public function canUpdate() : bool{
		...
		if($hasPlayer and $count < 15) return true;
		return false;
	}
		...
		if($this->canUpdate()){
			if($this->getDelay() <= 0){
				...
				for($i = 0; $i < $this->getSpawnCount(); $i++){
					...
					if(($target->getId() == Item::AIR or $target->getId() === 9 or $target->getId() === 8) && $ground->isTopFacingSurfaceSolid()){
						++$success;
			...
			}else $this->setDelay($this->getDelay() - 1);
		}
