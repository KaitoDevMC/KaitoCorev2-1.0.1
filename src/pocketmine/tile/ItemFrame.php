...
class ItemFrame extends Spawnable {
	...
	public function __construct(Level $level, CompoundTag $nbt){
		if(!isset($nbt->ItemRotation)) $nbt->ItemRotation = new ByteTag("ItemRotation", 0);

		if(!isset($nbt->ItemDropChance)) $nbt->ItemDropChance = new FloatTag("ItemDropChance", 1.0);
	...
	public function getItem() : Item{
		if(isset($this->namedtag->Item)) return Item::nbtDeserialize($this->namedtag->Item);
		else return Item::get(Item::AIR);
	}
	...
	public function setItem(Item $item = null){
		if($item !== null and $item->getId() !== Item::AIR) $this->namedtag->Item = $item->nbtSerialize(-1, "Item");
		else unset($this->namedtag->Item);
		$this->onChanged();
	}
	...
	public function getSpawnCompound(){
		...
		if($this->hasItem()){
			...
			if($this->getItem()->getId() === Item::FILLED_MAP){
				if(isset($this->namedtag->Map_UUID)) $tag->Map_UUID = $this->namedtag->Map_UUID;
			}
		}
