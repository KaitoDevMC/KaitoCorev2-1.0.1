...
class Sign extends Spawnable {
	...
	public function __construct(Level $level, CompoundTag $nbt){
		if(!isset($nbt->Text1)) $nbt->Text1 = new StringTag("Text1", "");
		if(!isset($nbt->Text2) or !($nbt->Text2 instanceof StringTag)) $nbt->Text2 = new StringTag("Text2", "");
		if(!isset($nbt->Text3) or !($nbt->Text3 instanceof StringTag)) $nbt->Text3 = new StringTag("Text3", "");
		if(!isset($nbt->Text4) or !($nbt->Text4 instanceof StringTag)) $nbt->Text4 = new StringTag("Text4", "");
	...
	public function setLine(int $index, string $line, bool $update = true){
		if($index < 0 or $index > 3) throw new \InvalidArgumentException("Index must be in the range 0-3!");
	...
	public function updateCompoundTag(CompoundTag $nbt, Player $player) : bool{
		if($nbt["id"] !== Tile::SIGN) return false;
		...
		if(!isset($this->namedtag->Creator) or $this->namedtag["Creator"] !== $player->getRawUniqueId()) $ev->setCancelled();
