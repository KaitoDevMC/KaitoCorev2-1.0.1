...
class Chest extends Spawnable implements InventoryHolder, Container, Nameable{
		...
		if(!$this->closed){
			...
			if($this->doubleInventory !== null){
				if($this->isPaired() and $this->level->isChunkLoaded($this->namedtag->pairx->getValue() >> 4, $this->namedtag->pairz->getValue() >> 4)){
					...
					if(($pair = $this->getPair()) !== null) $pair->doubleInventory = null;
				}
	...
	protected function getSlotIndex($index){
		foreach($this->namedtag->Items as $i => $slot){
			if((int) $slot["Slot"] === (int) $index) return (int) $i;
		}
	...
	public function getInventory(){
		if($this->isPaired() and $this->doubleInventory === null) $this->checkPairing();
		...
		if($this->isPaired() and !$this->getLevel()->isInLoadedTerrain(new Vector3($this->namedtag->pairx->getValue(), $this->y, $this->namedtag->pairz->getValue()))){
			...
			if($this->doubleInventory === null){
				if($pair->doubleInventory !== null){
					...
					if(($pair->x + ($pair->z << 15)) > ($this->x + ($this->z << 15))) $this->doubleInventory = $pair->doubleInventory = new DoubleChestInventory($pair, $this);
					else $this->doubleInventory = $pair->doubleInventory = new DoubleChestInventory($this, $pair);
				}
			}
		}else{
			$this->doubleInventory = null;
	...
	public function getPair(){
		if($this->isPaired()){
			...
			if($tile instanceof Chest) return $tile;
		}
	...
	public function pairWith(Chest $tile){
		if($this->isPaired() or $tile->isPaired()) return false;
