...
class ResourcePackManager {
	...
	public function __construct(Server $server, string $path){
		...
		if(!file_exists($this->path . "resource_packs.yml")){
			...
			if(file_exists($this->server->getFilePath() . "src/pocketmine/resources/resource_packs_$lang.yml")) $content = file_get_contents($file = $this->server->getFilePath() . "src/pocketmine/resources/resource_packs_$lang.yml");
			else $content = file_get_contents($file = $this->server->getFilePath() . "src/pocketmine/resources/resource_packs_eng.yml");
		...
		foreach($this->resourcePacksConfig->get("resource_stack", []) as $pos => $pack){
			...
			try{
				...
				if(!file_exists($packPath)) throw new ResourcePackException("File or directory not found");
				if(is_dir($packPath)) throw new ResourcePackException("Directory resource packs are unsupported");
