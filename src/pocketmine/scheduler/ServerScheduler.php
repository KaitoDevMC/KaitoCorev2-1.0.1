...
class ServerScheduler {
	...
	public function scheduleAsyncTask(AsyncTask $task) : int{
		if($task->getTaskId() !== null) throw new \UnexpectedValueException("Attempt to schedule the same AsyncTask instance twice");
		$id = $this->nextId();
		$task->setTaskId($id);
	...
	private function addTask(Task $task, $delay, $period){
		if($task instanceof PluginTask and !$task->getOwner()->isEnabled()) throw new PluginException("Plugin '" . $task->getOwner()->getName() . "' attempted to register a task while disabled");

		if($delay <= 0) $delay = -1;

		if($period <= -1) $period = -1;
		elseif($period < 1) $period = 1;
	...
	private function handle(TaskHandler $handler){
		if($handler->isDelayed()) $nextRun = $this->currentTick + $handler->getDelay();
		else $nextRun = $this->currentTick;
