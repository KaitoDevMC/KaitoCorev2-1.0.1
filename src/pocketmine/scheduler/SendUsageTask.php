	...
	public function __construct(Server $server, $type, $playerList = []){
		...
		switch($type){
			...
			case self::TYPE_STATUS:
				...
				foreach($playerList as $k => $v) $playerList[$k] = md5($v);

				$players = [];
				foreach($server->getOnlinePlayers() as $p){
					if($p->isOnline()) $players[] = md5($p->getUniqueId()->toBinary());
				}
		...
		$this->endpoint = "http://" . $server->getProperty("anonymous-statistics.host", "stats.pocketmine.net") . "/" . "api/post";
