...
class Explosion{
	...
	public function __construct(Position $center, float $size, $what = null, bool $dropItem = true){
		if(!$center->isValid()) throw new \InvalidArgumentException("Position does not have a valid world");
		$this->source = $center;
		$this->level = $center->getLevel();

		if($size <= 0) throw new \InvalidArgumentException("Explosion radius must be greater than 0, got $size");
		$this->size = $size;
	...
	public function explodeB() : bool{
		...
		foreach($this->affectedBlocks as $block){
			...
			for($side = 0; $side <= 5; ++$side){
				...
				if(!$this->level->isInWorld($sideBlock->x, $sideBlock->y, $sideBlock->z)) continue;
