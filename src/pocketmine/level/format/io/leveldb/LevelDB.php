...
class LevelDB extends BaseLevelProvider {
	...
	public function __construct(string $path, LevelTimings $timings = null){
		...
		if($levelData instanceof CompoundTag) $this->levelData = $levelData;
		else throw new LevelException("Invalid level.dat");
		...
		if(!isset($this->levelData->generatorOptions)) $this->levelData->generatorOptions = new StringTag("generatorOptions", "");
	}
	...
	public static function generate(string $path, string $name, $seed, string $generator, array $options = []){
		if(!file_exists($path . "/db")) mkdir($path . "/db", 0777, true);
	...
	protected function readChunk(int $chunkX, int $chunkZ) : ?Chunk{
		...
		if(!$this->chunkExists($chunkX, $chunkZ)) return null;
		...
		try{
			...
			for($y = Chunk::MAX_SUBCHUNKS - 1; $y >= 0; --$y){
				if($this->db->get($index . self::TAG_SUBCHUNK_PREFIX . chr($y)) !== false) break;
			}
