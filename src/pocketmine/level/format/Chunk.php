...
class Chunk{
    ...
    public function __construct(int $chunkX, int $chunkZ, array $subChunks = [], array $entities = [], array $tiles = [], string $biomeIds = "", array $heightMap = []){
        ...
        foreach($subChunks as $y => $subChunk){
            if($y < 0 or $y >= $this->height) throw new ChunkException("Invalid subchunk index $y!");
            if($subChunk->isEmpty()) $this->subChunks[$y] = $this->emptySubChunk;
            else $this->subChunks[$y] = $subChunk;
        }

        for($i = 0; $i < $this->height; ++$i){
            if(!isset($this->subChunks[$i])) $this->subChunks[$i] = $this->emptySubChunk;
        }
    ...
    public function setBlockData(int $x, int $y, int $z, int $data){
        if($this->getSubChunk($y >> 4)->setBlockData($x, $y & 0x0f, $z, $data)) $this->hasChanged = true;
    }
    ...
    public function setBlockExtraData(int $x, int $y, int $z, int $data){
        if($data === 0) unset($this->extraData[Chunk::chunkBlockHash($x, $y, $z)]);
        else $this->extraData[Chunk::chunkBlockHash($x, $y, $z)] = $data;
    ...
    public function setBlockSkyLight(int $x, int $y, int $z, int $level){
        if($this->getSubChunk($y >> 4)->setBlockSkyLight($x, $y & 0x0f, $z, $level)) $this->hasChanged = true;
    }
    ...
    public function setAllBlockSkyLight(int $level){
        $data = str_repeat(chr(($level & 0x0f) | ($level << 4)), 2048);
        for($y = $this->getHighestSubChunkIndex(); $y >= 0; --$y) $this->getSubChunk($y, true)->setBlockSkyLightArray($data);
    }
    ...
    public function setBlockLight(int $x, int $y, int $z, int $level){
        if($this->getSubChunk($y >> 4)->setBlockLight($x, $y & 0x0f, $z, $level)) $this->hasChanged = true;
    }
    ...
    public function setAllBlockLight(int $level){
        $data = str_repeat(chr(($level & 0x0f) | ($level << 4)), 2048);
        for($y = $this->getHighestSubChunkIndex(); $y >= 0; --$y) $this->getSubChunk($y, true)->setBlockLightArray($data);
    }
    ...
    public function recalculateHeightMap(){
        for($z = 0; $z < 16; ++$z){
            for($x = 0; $x < 16; ++$x) $this->recalculateHeightMapColumn($x, $z);
        }
    }
    ...
    public function recalculateHeightMapColumn(int $x, int $z) : int{
        ...
        for($y = $max; $y >= 0; --$y){
            if(Block::$lightFilter[$id = $this->getBlockId($x, $y, $z)] > 1 or Block::$diffusesSkyLight[$id]) break;
        }
    ...
    public function populateSkyLight(){
     ...
     //            for($y = $maxY; $y >= $heightMap; --$y) $this->setBlockSkyLight($x, $y, $z, 15);
    ...
    public function getBlockIdColumn(int $x, int $z) : string{
        $result = "";
        foreach($this->subChunks as $subChunk) $result .= $subChunk->getBlockIdColumn($x, $z);
        return $result;
    }
    ...
    public function getBlockDataColumn(int $x, int $z) : string{
        $result = "";
        foreach($this->subChunks as $subChunk) $result .= $subChunk->getBlockDataColumn($x, $z);
        return $result;
    }
    ...
    public function getBlockSkyLightColumn(int $x, int $z) : string{
        $result = "";
        foreach($this->subChunks as $subChunk) $result .= $subChunk->getSkyLightColumn($x, $z);
        return $result;
    }
    ...
    public function getBlockLightColumn(int $x, int $z) : string{
        $result = "";
        foreach($this->subChunks as $subChunk) $result .= $subChunk->getBlockLightColumn($x, $z);
        return $result;
    }
    ...
    public function addEntity(Entity $entity){
        ...
        if(!($entity instanceof Player) and $this->isInit) $this->hasChanged = true;
    }
    ...
    public function addTile(Tile $tile){
    	if($tile->closed) throw new \InvalidArgumentException("Attempted to add a garbage closed Tile to a chunk");
        $this->tiles[$tile->getId()] = $tile;
        if(isset($this->tileList[$index = (($tile->x & 0x0f) << 12) | (($tile->z & 0x0f) << 8) | ($tile->y & 0xff)]) and $this->tileList[$index] !== $tile) $this->tileList[$index]->close();
        $this->tileList[$index] = $tile;
        if($this->isInit) $this->hasChanged = true;
    }
    ...
    public function onUnload() : void{
        ...
        foreach($this->getTiles() as $tile) $tile->close();
    }
    ...
    public function getSubChunk(int $y, bool $generateNew = false) : SubChunk{
        if($y < 0 or $y >= $this->height) return $this->emptySubChunk;
        elseif($generateNew and $this->subChunks[$y] instanceof EmptySubChunk) $this->subChunks[$y] = new SubChunk();
    ...
    public function setSubChunk(int $y, SubChunk $subChunk = null, bool $allowEmpty = false) : bool{
        ...
        if($subChunk === null or ($subChunk->isEmpty() and !$allowEmpty)) $this->subChunks[$y] = $this->emptySubChunk;
        else $this->subChunks[$y] = $subChunk;
        $this->hasChanged = true;
        return true;
    }
    ...
    public function networkSerialize() : string{
        ...
        for($y = 0; $y < $subChunkCount; ++$y) $result .= $this->subChunks[$y]->networkSerialize();
        ...
        foreach($this->tiles as $tile){
			if($tile instanceof Spawnable) $result .= $tile->getSerializedSpawnCompound();
        }
    ...
    public function fastSerialize() : string{
        ...
        if($this->terrainGenerated){
            ...
            foreach($this->subChunks as $y => $subChunk){
                ...
                if($this->lightPopulated) $subChunks .= $subChunk->getSkyLightArray() . $subChunk->getBlockLightArray();
            }
            ...
            if($this->lightPopulated) $stream->put(pack("v*", ...$this->heightMap));
        }
    ...
    public static function fastDeserialize(string $data){
        ...
        if($terrainGenerated){
            ...
            if($lightPopulated) $heightMap = array_values(unpack("v*", $stream->get(512)));
        }
