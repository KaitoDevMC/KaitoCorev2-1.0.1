 ...
 *close
 *removeEntity
 * This program is free software: you can redistribute it and/or modifyTile
 * it under the terms of the GNU Lesser General Public License as published byAttempted to
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * @author vk.com/sanekmelkov and vk.com/kirillmineproytpopulateChunk
 *
 *addParticle
*/
...
use pocketmine\entity\{Lightning, XPOrb, Wither};
...
class Level implements ChunkManager, Metadatable{
    ...
    private $randomTickBlocks = [
        ...
        Block::CHORUS_FLOWER => ChorusFlower::class,
    ...
    public function setBlockTempData(Vector3 $pos, $data = null) {
        if($data == null and isset($this->blockTempData[self::blockHash($pos->x, $pos->y, $pos->z)])) unset($this->blockTempData[self::blockHash($pos->x, $pos->y, $pos->z)]);
        else $this->blockTempData[self::blockHash($pos->x, $pos->y, $pos->z)] = $data;
    }
    ...
    public function getBlockTempData(Vector3 $pos) {
        if(isset($this->blockTempData[self::blockHash($pos->x, $pos->y, $pos->z)])) return $this->blockTempData[self::blockHash($pos->x, $pos->y, $pos->z)];
        return 0;
    }
    ...
    public static function blockHash(int $x, int $y, int $z) {
        if($y < 0 or $y >= Level::Y_MAX) throw new \InvalidArgumentException("Y coordinate $y is out of range!");
    ...
    public static function generateChunkLoaderId(ChunkLoader $loader): int {
        if($loader->getLoaderId() === 0 or $loader->getLoaderId() === null) return ++self::$chunkLoaderCounter;
        else throw new \InvalidStateException("ChunkLoader has a loader id already assigned: " . $loader->getLoaderId());
    }
    ...
    public function __construct(Server $server, string $name, string $path, string $provider) {
        ...
        if (is_subclass_of($provider, LevelProvider::class, true)) $this->provider = new $provider($path, $this->timings);
        else throw new LevelException("Provider is not a subclass of LevelProvider");
        ...
        elseif($this->server->enderEnabled and $this->server->enderName == $this->folderName) $this->setDimension(self::DIMENSION_END);

        if($this->server->weatherEnabled and $this->getDimension() == self::DIMENSION_NORMAL) $this->weather->setCanCalculate(true);
    ...
    public function getWaterHeight(): int {
        if ($this->generator instanceof Generator) return $this->generator->getWaterHeight();
        return 0;
    }
    ...
    public function close() {
        if($this->closed) throw new \InvalidStateException("Tried to close a world which is already closed");
    ...
    public function addSound(Sound $sound, array $players = null){
        ...
        if(count($pk) > 0){
            if($players === null){
                foreach($pk as $e) $this->broadcastPacketToViewers($sound, $e);
            }else $this->server->batchPackets($players, $pk, false);
        }
    }
    ...
    public function addParticle(Particle $particle, array $players = null){
        ...
        if(count($pk) > 0){
            if($players === null){
                foreach($pk as $e) $this->broadcastPacketToViewers($particle, $e);
            }else $this->server->batchPackets($players, $pk, false);
        }
    }
    ...
    public function unload(bool $force = false) : bool{
        if($this->doingTick and !$force) throw new \InvalidStateException('Cannot unload a level during level tick');

        $ev = new LevelUnloadEvent($this);

        if ($this === $this->server->getDefaultLevel() and !$force) $ev->setCancelled(true);

        $this->server->getPluginManager()->callEvent($ev);

        if(!$force and $ev->isCancelled()) return false;

        $this->server->getLogger()->info($this->server->getLanguage()->translateString("pocketmine.level.unloading", [$this->getName()]));
        $defaultLevel = $this->server->getDefaultLevel();
        foreach ($this->getPlayers() as $player) {
            if($this === $defaultLevel or $defaultLevel === null) $player->close($player->getLeaveMessage(), 'Forced default level unload');
            else $player->teleport($defaultLevel->getSafeSpawn());
        }

        if($this === $defaultLevel) $this->server->setDefaultLevel(null);
    ...
    public function addChunkPacket(int $chunkX, int $chunkZ, DataPacket $packet){
        if(!isset($this->chunkPackets[$index = Level::chunkHash($chunkX, $chunkZ)])) $this->chunkPackets[$index] = [$packet];
        else $this->chunkPackets[$index][] = $packet;
    }
    ...
    public function registerChunkLoader(ChunkLoader $loader, int $chunkX, int $chunkZ, bool $autoLoad = true) {
        ...
        }elseif(isset($this->chunkLoaders[$chunkHash][$loaderId])) return;

        $this->chunkLoaders[$chunkHash][$loaderId] = $loader;
        if($loader instanceof Player) $this->playerLoaders[$chunkHash][$loaderId] = $loader;

        if(!isset($this->loaders[$loaderId])){
            $this->loaderCounter[$loaderId] = 1;
            $this->loaders[$loaderId] = $loader;
        }else ++$this->loaderCounter[$loaderId];

        $this->cancelUnloadChunkRequest($chunkX, $chunkZ);

        if($autoLoad) $this->loadChunk($chunkX, $chunkZ);
    }
    ...
    public function doTick(int $currentTick) {
        if($this->closed) throw new \InvalidStateException("Attempted to tick a Level which has been closed");
    ...
    protected function actuallyDoTick(int $currentTick) : void{
        if(!$this->stopTime){
            ...
            if($this->time === PHP_INT_MAX) $this->time = PHP_INT_MIN;
            else ++$this->time;
        }
        ...
        while($this->scheduledBlockUpdateQueue->count() > 0 and $this->scheduledBlockUpdateQueue->current()["priority"] <= $currentTick){
            ...
            if(!$this->isInLoadedTerrain($vec)) continue;
        ...
        while($this->neighbourBlockUpdateQueue->count() > 0){
            ...
            if(!$ev->isCancelled()) $block->onUpdate(self::BLOCK_UPDATE_NORMAL);
        }
        ...
        foreach ($this->updateEntities as $id => $entity) {
            if($entity->closed or !$entity->onUpdate($currentTick)) unset($this->updateEntities[$id]);
        }
        ...
        foreach($this->updateTiles as $id => $tile){
            if(!$tile->onUpdate()) unset($this->updateTiles[$id]);
        }
        ...
        if (count($this->changedBlocks) > 0) {
            if (count($this->players) > 0) {
                foreach ($this->changedBlocks as $index => $blocks) {
                    ...
                    }else $this->sendBlocks($this->getChunkPlayers($chunkX, $chunkZ), $blocks, UpdateBlockPacket::FLAG_ALL);
                }
            }else $this->chunkCache = [];
        ...
        if(count($this->globalPackets) > 0){
            if(count($this->players) > 0) $this->server->batchPackets($this->players, $this->globalPackets);
            $this->globalPackets = [];
        }
        ...
        foreach ($this->chunkPackets as $index => $entries) {
            ...
            if(count($chunkPlayers) > 0) $this->server->batchPackets($chunkPlayers, $entries, false, false);
        }
    ...
    public function checkSleep() {
        ...
        if($resetTime){
            ...
            if($time >= Level::TIME_NIGHT and $time < Level::TIME_SUNRISE){
                ...
                foreach($this->getPlayers() as $p) $p->stopSleep();
            }
        }
    }
    ...
    public function sendBlocks(array $target, array $blocks, $flags = UpdateBlockPacket::FLAG_NONE, bool $optimizeRebuilds = false){
        ...
        if ($optimizeRebuilds) {
            $chunks = [];
            foreach ($blocks as $b) {
                if(!($b instanceof Vector3)) throw new \TypeError("Expected Vector3 in blocks array, got " . (is_object($b) ? get_class($b) : gettype($b)));
    ...
    private function tickChunks() {
        if ($this->chunksPerTick <= 0 or count($this->loaders) === 0) return $this->chunkTickList = [];
        ...
        foreach ($this->loaders as $loader) {
            ...
            for ($chunk = 0; $chunk < $chunksPerLoader; ++$chunk) {
                ...
                if (!isset($this->chunkTickList[$hash]) and isset($this->chunks[$hash])) $this->chunkTickList[$hash] = -1;
            }
        }
        ...
        foreach ($this->chunkTickList as $index => $loaders) {
            ...
            if($loaders <= 0) unset($this->chunkTickList[$index]);
        ...
        if ($this->clearChunksOnTick) $this->chunkTickList = [];
    }
    ...
    public function save(bool $force = false) : bool{
        ...
        if (!$this->getAutoSave() and !$force) return false;

        $this->server->getPluginManager()->callEvent(new LevelSaveEvent($this));

        $this->provider->setTime($this->time);
        $this->saveChunks();
        if ($this->provider instanceof BaseLevelProvider) $this->provider->saveLevelData();
        return true;
    }
    ...
    public function scheduleNeighbourBlockUpdates(Vector3 $pos){
        ...
        for($i = 0; $i <= 5; ++$i){
            $side = $pos->getSide($i);
            if($this->isInWorld($side->x, $side->y, $side->z)) $this->neighbourBlockUpdateQueue->enqueue(Level::blockHash($side->x, $side->y, $side->z));
        }
    }
        ...
        if($targetFirst){
            for($z = $minZ; $z <= $maxZ; ++$z){
                for($x = $minX; $x <= $maxX; ++$x){
                    for($y = $minY; $y <= $maxY; ++$y){
                        $block = $this->getBlockAt($x, $y, $z);
                        if(!$block->canPassThrough() and $block->collidesWithBB($bb)) return [$block];
                    }
                }
            }
        }else{
            for($z = $minZ; $z <= $maxZ; ++$z){
                for($x = $minX; $x <= $maxX; ++$x){
                    for($y = $minY; $y <= $maxY; ++$y){
                        $block = $this->getBlockAt($x, $y, $z);
                        if(!$block->canPassThrough() and $block->collidesWithBB($bb)) $collides[] = $block;
                    }
                }
            }
        }
    ...
    public function isFullBlock(Vector3 $pos) : bool{
        if($pos instanceof Block){
            if ($pos->isSolid()) return true;
            $bb = $pos->getBoundingBox();
        }else $bb = $this->getBlock($pos)->getBoundingBox();
                ...
                for($y = $minY; $y <= $maxY; ++$y){
                    ...
                    if(!$block->canPassThrough()){
                        foreach($block->getCollisionBoxes() as $blockBB){
                            if($blockBB->intersectsWith($bb))$collides[] = $blockBB;
                        }
                    }
                }
            }
    ...
    public function getFullLightAt(int $x, int $y, int $z) : int{
        ...
        if($skyLight < 15) return max($skyLight, $this->getBlockLightAt($x, $y, $z));
    ...
    public function getBlockAt(int $x, int $y, int $z, bool $cached = true, bool $addToCache = true) : Block{
        ...
        if($this->isInWorld($x, $y, $z)){
            ...
            if($cached and isset($this->blockCache[$chunkHash][$blockHash])) return $this->blockCache[$chunkHash][$blockHash];

            $chunk = $this->chunks[$chunkHash] ?? null;
            if($chunk !== null) $fullState = $chunk->getFullBlock($x & 0x0f, $y, $z & 0x0f);
            else $addToCache = false;
        }
        ...
        if($addToCache and $blockHash !== null) $this->blockCache[$chunkHash][$blockHash] = $block;
    ...
    public function updateBlockSkyLight(int $x, int $y, int $z) {
        ...
        }else $newHeightMap = $oldHeightMap;
        ...
        }else $update->setAndUpdateLight($x, $y, $z, max(0, $this->getHighestAdjacentBlockSkyLight($x, $y, $z) - Block::$lightFilter[$sourceId]));
    ...
    public function getHighestAdjacentBlockLight(int $x, int $y, int $z): int {
        return max([
            ...
            $this->getBlockLightAt($x, $y, $z - 1)
                                            ...
                                            /*WARNING, если тени залагают, расскоментируйте эти две функции*/
    public function updateBlockLight(int $x, int $y, int $z) {
        ...
        if($this->blockLightUpdate === null) $this->blockLightUpdate = new BlockLightUpdate($this);
    ...
    public function setBlock(Vector3 $pos, Block $block, bool $direct = false, bool $update = true) : bool{
        $pos = $pos->floor();
        if(!$this->isInWorld($pos->x, $pos->y, $pos->z)) return false;

        $this->timings->setBlock->startTiming();

        if($this->getChunkAtPosition($pos, true)->setBlock($pos->x & 0x0f, $pos->y, $pos->z & 0x0f, $block->getId(), $block->getDamage())){
            if(!($pos instanceof Position)) $pos = $this->temporalPosition->setComponents($pos->x, $pos->y, $pos->z);
            ...
            if($direct){
                ...
                if(!isset($this->changedBlocks[$chunkHash])) $this->changedBlocks[$chunkHash] = [];

                $this->changedBlocks[$chunkHash][$blockHash] = $block;
            }

            foreach($this->getChunkLoaders($pos->x >> 4, $pos->z >> 4) as $loader) $loader->onBlockChanged($block);
            ...
            if($update){
                ...
                $this->server->getPluginManager()->callEvent($ev = new BlockUpdateEvent($block));
                if (!$ev->isCancelled()) {
                    foreach ($this->getNearbyEntities(new AxisAlignedBB($block->x - 1, $block->y - 1, $block->z - 1, $block->x + 1, $block->y + 1, $block->z + 1)) as $entity) $entity->onNearbyBlockChange();
    ...
    public function checkSpawnProtection(Player $player, Vector3 $vector) : bool{
        if(!$player->hasPermission("pocketmine.spawnprotect.bypass") and ($distance = $this->server->getSpawnRadius()) > -1){
            if(count($this->server->getOps()->getAll()) > 0 and (new Vector2($vector->x, $vector->z))->distance(new Vector2($this->getSpawnLocation()->x, $this->getSpawnLocation()->z)) <= $distance) return true;
        }
    ...
    public function useBreakOn(Vector3 $vector, Item &$item = null, Player $player = null, bool $createParticles = false): bool {
        ...
        if ($player !== null) {
            ...
            if($target instanceof Air or ($player->isSurvival() and !$target->isBreakable($item)) or $player->isSpectator()){
                ...
                if(count($this->server->getOps()->getAll()) > 0 and (new Vector2($target->x, $target->z))->distance(new Vector2($this->getSpawnLocation()->x, $this->getSpawnLocation()->z)) <= $distance) $ev->setCancelled();
            }
            $this->server->getPluginManager()->callEvent($ev);
            ...
            if ($player->isSurvival() and $this->getServer()->expEnabled) {
                ...
                if(!$item->hasEnchantment(Enchantment::TYPE_MINING_SILK_TOUCH)){
                    switch($target->getId()){
                        case Block::COAL_ORE: $exp = mt_rand(0, 2); break;
                        ...
                        case Block::GLOWING_REDSTONE_ORE:
                            $exp = mt_rand(1, 5);
                        break;
                        case Block::MONSTER_SPAWNER: $exp = mt_rand(15, 43); break;
                    }
                }
        ...
        if($tile !== null){
            if($tile instanceof Container){
                if($tile instanceof Chest){
                    $tile->getInventory()->removeAllViewers();
                    $tile->unpair();
                }
                ...
                if($player !== null && $player->getTransactionQueue() !== null) $player->getTransactionQueue()->execute();
        ...
        $item->useOn($target);
        if($item->isTool() and $item->getDamage() >= $item->getMaxDurability()) $item = Item::get(Item::AIR, 0, 0);

        if($player === null or $player->isSurvival()){
            foreach($drops as $drop){
                if($drop->getCount() > 0) $this->dropItem($vector->add(0.5, 0.5, 0.5), $drop);
            }
        }
   ...
   public function useItemOn(Vector3 $vector, Item &$item, int $face, float $fx = 0.0, float $fy = 0.0, float $fz = 0.0, Player $player = null): bool{
        ...
        if($target->getId() === 0) return false;
        ...
        if($player !== null){
            ...
            if(!$ev->isCancelled()){
                ...
                if(!$player->isSneaking()) {
                    if ($target->canBeActivated() and $target->onActivate($item, $player)) {
        ...
        } elseif ($target->canBeActivated() and $target->onActivate($item, $player)) return true;
        ...
        if ($hand->isSolid() and $hand->getBoundingBox() !== null) {
            ...
            if ($player !== null) {
                if(($diff = $player->getNextPosition()->subtract($player->getPosition())) and $diff->lengthSquared() > 0.00001){
                    if($hand->getBoundingBox()->intersectsWith($player->getBoundingBox()->getOffsetBoundingBox($diff->x, $diff->y, $diff->z))) ++$realCount;
                }
            }

            if($realCount > 0) return false; //Entity in block
        }
        ...
        if($player !== null){
            if(!$player->isOp() and ($distance = $this->server->getSpawnRadius()) > -1) if(count($this->server->getOps()->getAll()) > 0 and (new Vector2($target->x, $target->z))->distance(new Vector2($this->getSpawnLocation()->x, $this->getSpawnLocation()->z)) <= $distance) $ev->setCancelled();
            $this->server->getPluginManager()->callEvent($ev = new BlockPlaceEvent($player, $hand, $block, $target, $item));
            if($ev->isCancelled()) return false;
        }

        if(!$hand->place($item, $block, $target, $face, $fx, $fy, $fz, $player)) return false;
            ...
            for($x = $minX; $x <= $maxX; ++$x){
                for($z = $minZ; $z <= $maxZ; ++$z){
                    foreach($this->getChunkEntities($x, $z) as $ent){
                        ...
                        if($ent->canBeCollidedWith() and ($entity === null or ($ent !== $entity and $entity->canCollideWith($ent))) and $ent->boundingBox->intersectsWith($bb)) $nearby[] = $ent;
                    }
                }
            }
        }
    ...
    public function getNearbyEntities(AxisAlignedBB $bb, Entity $entity = null) : array{
        ...
        for($x = $minX; $x <= $maxX; ++$x){
            for($z = $minZ; $z <= $maxZ; ++$z){
                foreach($this->getChunkEntities($x, $z) as $ent){
                    if($ent instanceof Player and $ent->isSpectator()) continue;
                    if($ent !== $entity and $ent->boundingBox->intersectsWith($bb)) $nearby[] = $ent;
                }
            }
        }
        ...
        for($x = $minX; $x <= $maxX; ++$x){
            for($z = $minZ; $z <= $maxZ; ++$z){
                foreach($this->getChunkEntities($x, $z) as $entity){
                    ...
                    if(!($entity instanceof $entityType) or $entity->isClosed() or (!$includeDead and !$entity->isAlive())) continue;
    ...
    public function getNearbyExperienceOrb(AxisAlignedBB $bb): array {
        ...
        foreach ($this->getNearbyEntities($bb) as $entity) {
            if ($entity instanceof XPOrb) $nearby[] = $entity;
        }
    ...
    public function getTileAt(int $x, int $y, int $z) : ?Tile{
        ...
        if ($chunk !== null) return $chunk->getTile($x & 0x0f, $y, $z & 0x0f);
    ...
    public function setBlockIdAt(int $x, int $y, int $z, int $id) {
        if(!$this->isInWorld($x, $y, $z)) return;
     ...
     * @return int
     */
    public function getNormalId(int $x, int $z): int {
        return $this->getChunk($x >> 4, $z >> 4, true)->getNormalId($x & 0x0f, $z & 0x0f);
    }
     ...
     * @param int $NormalId
     */
    public function setNormalId(int $x, int $z, int $NormalId) {
        $this->getChunk($x >> 4, $z >> 4, true)->setNormalId($x & 0x0f, $z & 0x0f, $NormalId);
    }
    ...
    public function generateChunkCallback(int $x, int $z, ?Chunk $chunk) {
        ...
        if (isset($this->chunkPopulationQueue[$index = Level::chunkHash($x, $z)])) {
            ...
            if($chunk !== null) $this->setChunk($x, $z, $chunk, false);
        }elseif($chunk !== null){
            $this->setChunk($x, $z, $chunk, false);
        }
    ...
    public function spawnXPOrb(Vector3 $pos, int $exp = 1) {
        if($exp > 0){
            $expOrb = new XPOrb($this, new CompoundTag("", [
                "Pos" => new ListTag("Pos", [
                    new DoubleTag("", $pos->getX()),
    ...
    public function spawnWither(Vector3 $pos, Level $level){
        $wither = new Wither($level, new CompoundTag("", ["Pos" => new ListTag("Pos", [new DoubleTag("", $pos->x),new DoubleTag("", $pos->y),new DoubleTag("", $pos->z)]),"Motion" => new ListTag("Motion", [ new DoubleTag("", 0),new DoubleTag("", 0),new DoubleTag("", 0)]),"Rotation" => new ListTag("Rotation", [new FloatTag("", 90),new FloatTag("", 0)]),]));
        $wither->spawnToAll();
        return $wither;
    }
    ...
    public function requestChunk(int $x, int $z, Player $player){
        ...
        if (!isset($this->chunkSendQueue[$index])) $this->chunkSendQueue[$index] = [];
    ...
    private function sendChunkFromCache($x, $z) {
        if (isset($this->chunkSendQueue[$index = Level::chunkHash($x, $z)])) {
            foreach ($this->chunkSendQueue[$index] as $player) {
                /** @var Player $player */
                if ($player->isConnected() and isset($player->usedChunks[$index])) $player->sendChunk($x, $z, $this->chunkCache[$index]);
            }
    ...
    public function removeEntity(Entity $entity) {
        if ($entity->getLevel() !== $this) throw new LevelException('Invalid Entity level');
    ...
    public function addEntity(Entity $entity) {
        ...
        if ($entity instanceof Player) $this->players[$entity->getId()] = $entity;
    ...
    public function addTile(Tile $tile) {
        if($tile->isClosed()) throw new \InvalidArgumentException("Attempted to add a garbage closed Tile to world");
        if ($tile->getLevel() !== $this) throw new LevelException("Invalid Tile level");

        $chunkX = $tile->getFloorX() >> 4;
        $chunkZ = $tile->getFloorZ() >> 4;

        if(isset($this->chunks[$hash = Level::chunkHash($chunkX, $chunkZ)])) $this->chunks[$hash]->addTile($tile);
        else throw new \InvalidStateException("Attempted to create tile " . get_class($tile) . " in unloaded chunk $chunkX $chunkZ");
    ...
    public function removeTile(Tile $tile) {
        if ($tile->getLevel() !== $this) throw new LevelException("Invalid Tile world");
        ...
        if(isset($this->chunks[$hash = Level::chunkHash($chunkX, $chunkZ)])) $this->chunks[$hash]->removeTile($tile);
        $this->clearChunkCache($chunkX, $chunkZ);
    }
    ...
    public function loadChunk(int $x, int $z, bool $create = true) : bool{
        if(isset($this->chunks[$index = Level::chunkHash($x, $z)])) return true;
        ...
        if($chunk === null and $create) $chunk = new Chunk($x, $z);
        ...
        // if(!$chunk->isLightPopulated() and $chunk->isPopulated() and $this->getServer()->getProperty('chunk-ticking.light-updates', false)) $this->getServer()->getScheduler()->scheduleAsyncTask(new LightPopulationTask($this, $chunk));
    ...
    public function unloadChunkRequest(int $x, int $z, bool $safe = true){
        if(($safe and $this->isChunkInUse($x, $z)) or $this->isSpawnChunk($x, $z)) return false;
    ...
    public function unloadChunk(int $x, int $z, bool $safe = true, bool $trySave = true) : bool{
        if($safe and $this->isChunkInUse($x, $z)) return false;
        ...
        if ($chunk !== null) {
            ...
            foreach($this->getChunkLoaders($x, $z) as $loader) $loader->onChunkUnloaded($chunk);
    ...
    public function getSafeSpawn(?Vector3 $spawn = null) : Position{
        if(!($spawn instanceof Vector3) or $spawn->y < 1) $spawn = $this->getSpawnLocation();
        ...
        if($chunk !== null and $chunk->isGenerated()){
            ...
            for(; $y >= 0 and $y < $max; ++$y){
                if(!$this->isFullBlock($this->getBlockAt($x, $y + 1, $z))){
                    if(!$this->isFullBlock($this->getBlockAt($x, $y, $z))) return new Position($spawn->x, $y === (int) $spawn->y ? $spawn->y : $y, $spawn->z, $this);
                }else ++$y;
            }
    ...
    public function populateChunk(int $x, int $z, bool $force = false): bool {
        if(isset($this->chunkPopulationQueue[$index = Level::chunkHash($x, $z)]) or (count($this->chunkPopulationQueue) >= $this->chunkPopulationQueueSize and !$force)) return false;
        for($xx = -1; $xx <= 1; ++$xx){
            for($zz = -1; $zz <= 1; ++$zz){
                if(isset($this->chunkPopulationLock[Level::chunkHash($x + $xx, $z + $zz)])) return false;
            }
        }
        ...
        if (!$this->getChunk($x, $z, true)->isPopulated()) {
            ...
            $this->server->getScheduler()->scheduleAsyncTask(new PopulationTask($this, $this->getChunk($x, $z, true)));
    ...
    public function doChunkGarbageCollection() {
        ...
        foreach($this->chunks as $index => $chunk){
            if(!isset($this->unloadQueue[$index])){
                Level::getXZ($index, $X, $Z);
                if(!$this->isSpawnChunk($X, $Z)) $this->unloadChunkRequest($X, $Z, true);
            }
