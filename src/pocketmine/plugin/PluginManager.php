...
class PluginManager {
	...
	public function registerInterface($loaderName){
		if(is_subclass_of($loaderName, PluginLoader::class)) $loader = new $loaderName($this->server);
		else return false;
			...
			if(preg_match($loader->getPluginFilters(), basename($path)) > 0){
				...
				if($description instanceof PluginDescription){
					if(($plugin = $loader->loadPlugin($path)) instanceof Plugin){
						...
						if(sizeof($pluginCommands) > 0) $this->commandMap->registerAll($plugin->getDescription()->getName(), $pluginCommands);
			...
			while(sizeof($plugins) > 0){
				...
				foreach($plugins as $name => $file){
					...
					if(isset($softDependencies[$name])){
						...
						if(sizeof($softDependencies[$name]) === 0){
	...
	public function unsubscribeFromPermission($permission, Permissible $permissible){
		if(isset($this->permSubs[$permission])){
			...
			if(sizeof($this->permSubs[$permission]) === 0){
	...
	public function registerEvents(Listener $listener, Plugin $plugin){
		...
		foreach($reflection->getMethods(\ReflectionMethod::IS_PUBLIC) as $method){
			if(!$method->isStatic() and $method->getDeclaringClass()->implementsInterface(Listener::class)){
				...
				if(sizeof($parameters) !== 1) continue;
				try{
				...
				if($eventClass === null or !$eventClass->isSubclassOf(Event::class)) continue;
	...
	public function registerEvent($event, Listener $listener, $priority, EventExecutor $executor, Plugin $plugin, $ignoreCancelled = false){
		if(!is_subclass_of($event, Event::class)) throw new PluginException($event . " is not an Event");
		$class = new \ReflectionClass($event);
		if($class->isAbstract()) throw new PluginException($event . " is an abstract Event");
		if($class->getProperty("handlerList")->getDeclaringClass()->getName() !== $event) throw new PluginException($event . " does not have a handler list");

		if(!$plugin->isEnabled()) throw new PluginException("Plugin attempted to register " . $event . " while not enabled");
	...
	private function getEventListeners($event){
		if($event::$handlerList === null) $event::$handlerList = new HandlerList();
