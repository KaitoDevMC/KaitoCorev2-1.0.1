...
abstract class PluginBase implements Plugin {
	...
	public function saveResource($filename, $replace = false){
		...
		if(($resource = $this->getResource($filename)) === null) return false;

		$out = $this->dataFolder . $filename;
		if(!file_exists(dirname($out))) mkdir(dirname($out), 0755, true);

		if(file_exists($out) and $replace !== true) return false;
	...
	public function getResources(){
		...
		if(is_dir($this->file . "resources/")){
			foreach(new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator($this->file . "resources/")) as $resource){
				if($resource->isFile()) $resources[] = $resource;
			}
		}
	...
	public function getConfig(){
		if(!isset($this->config)) $this->reloadConfig();
	...
	public function saveConfig(){
		if($this->getConfig()->save() === false) $this->getLogger()->critical("Could not save config to " . $this->configFile);
	}
	...
	public function saveDefaultConfig(){
		if(!file_exists($this->configFile)) $this->saveResource("config.yml", false);
	}
	...
	public function reloadConfig(){
		if(!$this->saveDefaultConfig()) @mkdir($this->dataFolder);
