 ...
 *
 *
 * This program is free software: you can redistribute it and/or modifyclient
 * it under the terms of the GNU Lesser General Public License as published byisConnected
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * @author vk.com/sanekmelkov and vk.com/kirillmineproytXPOrb
 *
 *
*/
...
abstract class Projectile extends Entity{
	...
	public function __construct(Level $level, CompoundTag $nbt, Entity $shootingEntity = null){
		...
		if($shootingEntity !== null) $this->setOwningEntity($shootingEntity);
	}
	...
	public function attack($damage, EntityDamageEvent $source){
		if($source->getCause() === EntityDamageEvent::CAUSE_VOID) parent::attack($damage, $source);
	}
	...
	protected function initEntity(){
		...
		if(isset($this->namedtag->damage)) $this->damage = $this->namedtag["damage"];
	}
	...
	public function onCollideWithEntity(Entity $entity){
		...
		if($this instanceof WitherSkull){
			$this->close();
		...
		if($this->getOwningEntity() === null) $ev = new EntityDamageByEntityEvent($this, $entity, EntityDamageEvent::CAUSE_PROJECTILE, $damage);
		else $ev = new EntityDamageByChildEntityEvent($this->getOwningEntity(), $this, $entity, EntityDamageEvent::CAUSE_PROJECTILE, $damage);

		if($entity->attack($ev->getFinalDamage(), $ev) === true){
			if($this instanceof Arrow and $this->getPotionId() != 0){
				foreach(Potion::getEffectsById($this->getPotionId() - 1) as $effect) $entity->addEffect($effect->setDuration($effect->getDuration() / 8));
			}
			$ev->useArmors();
		}
		...
		if($this->fireTicks > 0){
			$this->server->getPluginManager()->callEvent($ev = new EntityCombustByEntityEvent($this, $entity, 5));
	...
	public function onUpdate($currentTick){
		...
		if($this->isAlive()){
			...
			if(!$this->isCollided) $this->motionY -= $this->gravity;
			...
			foreach($list as $entity){
				if(($entity->getId() === $this->getOwningEntityId() and $this->ticksLived < 5)) continue;
			...
			if(!$this->hadCollision or abs($this->motionX) > 0.00001 or abs($this->motionY) > 0.00001 or abs($this->motionZ) > 0.00001){
				...
				$this->pitch = (atan2($this->motionY, sqrt(($this->motionX ** 2) + ($this->motionZ ** 2))) * 180 / M_PI);
				$hasUpdate = true;
			}
