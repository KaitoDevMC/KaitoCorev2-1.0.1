...
use pocketmine\event\entity\{EntityDamageByEntityEvent, EntityDamageEvent, EntityDamageByChildEntityEvent};
...
class MinecartTNT extends Animal {
	...
	public function getDrops(){
		return [ItemItem::get(328, 0, 1), ItemItem::get(46, 0, 1)];
	}
	...
	public function onUpdate($currentTick){
		...
		if($this->isOnGround()){
			if($this->fallDistance > 0){
				...
				if($this->willMove()){
					if($this->owner != null){
						if((!$this->owner instanceof Player)or(!$this->owner->isSurvival())or($this->distance($this->owner) > 2)) $this->owner = null;
					}
					...
					if(($this->owner != null)and($this->owner instanceof Player)and($this->owner->isSurvival())and(($this->distance($this->owner) < 2)and($this->distance($this->owner) > 1))){
						if(($this->owner->x - $this->x) > $this->step) $x = $this->step;
						elseif(($this->owner->x - $this->x) < -$this->step) $x = -$this->step;
						if(($this->owner->z - $this->z) > $this->step) $z = $this->step;
						elseif(($this->owner->z - $this->z) < -$this->step) $z = -$this->step;
		...
		if((($x != 0)or($y != 0)or($z != 0))and($this->owner != null)) $this->setMotion(new Vector3($x, $y, $z));
	...
	public function attack($damage, EntityDamageEvent $source){
		...
		if(2 >= ($this->getHealth() - $damage) and $source instanceof EntityDamageByChildEntityEvent and $source->getChild() instanceof Arrow and $source->getChild()->isOnFire()){
