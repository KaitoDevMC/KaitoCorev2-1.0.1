...
class Sheep extends Animal implements Colorable {
		...
		if($cause instanceof EntityDamageByEntityEvent){
			...
			if($damager instanceof Player){
				$lootingL = $damager->getItemInHand()->getEnchantmentLevel(14);
					if($damager->getItemInHand()->hasEnchantment(13)){
						$drops = [ItemItem::get(ItemItem::WOOL, $this->getColor(), mt_rand(1, 3 + $lootingL)), ItemItem::get(ItemItem::COOKED_MUTTON, 0, mt_rand(1, 3 + $lootingL))];
						return $drops;
					}else{
						$drops = [ItemItem::get(ItemItem::WOOL, $this->getColor(), mt_rand(1, 3 + $lootingL)), ItemItem::get(ItemItem::RAW_MUTTON, 0, mt_rand(1, 3 + $lootingL))];
						return $drops;
				}			
			}
		}
	...
	public function isSheared() : bool{
		return Entity::getDataFlag(Entity::DATA_FLAGS, Entity::DATA_FLAG_SHEARED);
	}
