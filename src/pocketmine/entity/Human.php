...
class Human extends Creature implements ProjectileSource, InventoryHolder {
	...
	public function jump() : void{
		parent::jump();
		if($this->isSprinting()) $this->exhaust(0.8, PlayerExhaustEvent::CAUSE_SPRINT_JUMPING);
		else $this->exhaust(0.2, PlayerExhaustEvent::CAUSE_JUMPING);
	}
	...
	public function setFood(float $new){
		...
		foreach([17, 6, 0] as $bound){
			if(($old > $bound) !== ($new > $bound)) $reset = true;
		}
		if(isset($reset)){
	...
	public function setTotalXp(int $xp, bool $syncLevel = false) : bool{
		$xp &= 0x7fffffff;
		if($xp === $this->totalXp) return false;
		if(!$syncLevel){
	...
	public function getTransactionQueue(){
		...
		if($this->transactionQueue === null) $this->transactionQueue = new SimpleTransactionQueue($this);
	...
	protected function initEntity(){
		...
		if($this instanceof Player){
			...
			if(isset($this->namedtag->Skin) and $this->namedtag->Skin instanceof CompoundTag) $this->setSkin($this->namedtag->Skin["Data"], $this->namedtag->Skin["Name"]);
		...
		if(isset($this->namedtag->OffHandItem) && $this->namedtag->OffHandItem instanceof CompoundTag){
		    if($this->offhandInventory === null) $this->offhandInventory = new OffhandInventory($this);
		...
		if(!isset($this->namedtag->foodLevel) or !($this->namedtag->foodLevel instanceof IntTag)) $this->namedtag->foodLevel = new IntTag("foodLevel", $this->getFood());
		else $this->setFood($this->namedtag["foodLevel"]);

		if(!isset($this->namedtag->foodExhaustionLevel) or !($this->namedtag->foodExhaustionLevel instanceof IntTag)) $this->namedtag->foodExhaustionLevel = new FloatTag("foodExhaustionLevel", $this->getExhaustion());
		else $this->setExhaustion($this->namedtag["foodExhaustionLevel"]);

		if(!isset($this->namedtag->foodSaturationLevel) or !($this->namedtag->foodSaturationLevel instanceof IntTag)) $this->namedtag->foodSaturationLevel = new FloatTag("foodSaturationLevel", $this->getSaturation());
		else $this->setSaturation($this->namedtag["foodSaturationLevel"]);

		if(!isset($this->namedtag->foodTickTimer) or !($this->namedtag->foodTickTimer instanceof IntTag)) $this->namedtag->foodTickTimer = new IntTag("foodTickTimer", $this->foodTickTimer);
		else $this->foodTickTimer = $this->namedtag["foodTickTimer"];

		if(!isset($this->namedtag->XpLevel) or !($this->namedtag->XpLevel instanceof IntTag)) $this->namedtag->XpLevel = new IntTag("XpLevel", 0);
		$this->setXpLevel($this->namedtag["XpLevel"]);

		if(!isset($this->namedtag->XpP) or !($this->namedtag->XpP instanceof FloatTag)) $this->namedtag->XpP = new FloatTag("XpP", 0);
		$this->setXpProgress($this->namedtag["XpP"]);

		if(!isset($this->namedtag->XpTotal) or !($this->namedtag->XpTotal instanceof IntTag)) $this->namedtag->XpTotal = new IntTag("XpTotal", 0);
		$this->totalXp = $this->namedtag["XpTotal"];

		if(!isset($this->namedtag->XpSeed) or !($this->namedtag->XpSeed instanceof IntTag)) $this->namedtag->XpSeed = new IntTag("XpSeed", mt_rand(PHP_INT_MIN, PHP_INT_MAX));
	...
	public function entityBaseTick($tickDiff = 1, $EnchantL = 0){
		if($this->getInventory() instanceof PlayerInventory) $EnchantL = $this->getInventory()->getHelmet()->getEnchantmentLevel(Enchantment::TYPE_WATER_BREATHING);
	...
	public function attack($damage, EntityDamageEvent $source){
		if($source->getCause() !== EntityDamageEvent::CAUSE_VOID){
			...
			if($this->getAbsorption() > 0) $damage -= $this->getAbsorption();

			if(!$source->isCancelled() && $damage > 0 && ($this->getHealth() - $damage) <= 0){
				if($this->tryUseTotem()) return;
			}
		}
	...
	private function tryUseTotem() : bool{
		if($this->getOffhandInventory()->getItemInOffhand() instanceof Totem || $this->getInventory()->getItemInHand() instanceof Totem){
			$this->server->getPluginManager()->callEvent($event = new EntityConsumeTotemEvent($this));
			if(!$event->isCancelled()){
				...
				if($this instanceof Player) $viewers = array_merge($viewers, [$this]);
				...
				if($this->getOffhandInventory()->getItemInOffhand() instanceof Totem) $this->getOffhandInventory()->setItemInOffhand(ItemItem::get(ItemIds::AIR));
				else $this->getInventory()->setItemInHand(ItemItem::get(ItemIds::AIR));
	...
	public function doFoodTick($tickDiff = 1){
		if($this->isAlive()){
			...
			if($this->foodTickTimer >= 80) $this->foodTickTimer = 0;
			...
			if($difficulty === 0 and $this->foodTickTimer % 10 === 0){ //Peaceful
				...
				if($this->foodTickTimer % 20 === 0 and $health < $this->getMaxHealth()) $this->heal(1, new EntityRegainHealthEvent($this, 1, EntityRegainHealthEvent::CAUSE_SATURATION));
			}
			...
			if($this->foodTickTimer === 0){
				if($food >= 18){
					...
					if(($difficulty === 1 and $health > 10) or ($difficulty === 2 and $health > 1) or $difficulty === 3) $this->attack(1, new EntityDamageEvent($this, EntityDamageEvent::CAUSE_STARVATION, 1));
				}
			}

			if($food <= 6) $this->setSprinting(false);
		}
	}
	...
	public function getDrops(){
		...
		if($this->inventory !== null){
			foreach($this->inventory->getContents() as $item) $drops[] = $item;
		}
	...
	public function saveNBT(){
		...
		if($this->offhandInventory !== null)$this->namedtag->OffHandItem = $this->getOffhandInventory()->getItemInOffhand()->nbtSerialize(0, "OffHandItem");
		...
		if($this->inventory !== null){
			...
			for($slot = $this->inventory->getHotbarSize(); $slot < $slotCount; ++$slot){
				...
				if($item->getId() !== ItemItem::AIR) $this->namedtag->Inventory[$slot] = $item->nbtSerialize($slot);
			}

			//Armor
			for($slot = 100; $slot < 104; ++$slot){
				$item = $this->inventory->getItem($this->inventory->getSize() + $slot - 100);
				if($item instanceof ItemItem and $item->getId() !== ItemItem::AIR) $this->namedtag->Inventory[$slot] = $item->nbtSerialize($slot);
			}
		}
		...
		if($this->enderChestInventory !== null){
			for($slot = 0; $slot < $this->enderChestInventory->getSize(); $slot++){
				if(($item = $this->enderChestInventory->getItem($slot)) instanceof ItemItem) $this->namedtag->EnderChestInventory[$slot] = $item->nbtSerialize($slot);
			}
		}
	...
	public function spawnTo(Player $player){
		if(strlen($this->skin) < 64 * 32 * 4){
			...
			if(!($this instanceof Player)) $this->server->updatePlayerListData($this->getUniqueId(), $this->getId(), $this->getName(), $this->skinId, $this->skin, [$player]);
	...
	public function close(){
		if(!$this->closed){
			if($this->getFloatingInventory() instanceof FloatingInventory){
				foreach($this->getFloatingInventory()->getContents() as $craftingItem) $this->level->dropItem($this, $craftingItem);
			}else $this->server->getLogger()->debug("Attempted to drop a null crafting inventory\n");
			if($this->inventory !== null){
