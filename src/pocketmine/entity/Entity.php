...
//		self::DATA_BOUNDING_BOX_HEIGHT => [self::DATA_TYPE_FLOAT, 1.8],
	...
	public function __construct(Level $level, CompoundTag $nbt){
		...
		if($this->eyeHeight === null) $this->eyeHeight = $this->height / 2 + 0.1;
		...
		if($this->chunk === null) throw new \InvalidStateException("Cannot create entities in unloaded chunks");
		...
		if(isset($this->namedtag->Motion)) $this->setMotion($this->temporalVector->setComponents($this->namedtag["Motion"][0], $this->namedtag["Motion"][1], $this->namedtag["Motion"][2]));
		else $this->setMotion($this->temporalVector->setComponents(0, 0, 0));

		$this->resetLastMovements();

		assert(!is_nan($this->x) and !is_infinite($this->x) and !is_nan($this->y) and !is_infinite($this->y) and !is_nan($this->z) and !is_infinite($this->z));

		if(!isset($this->namedtag->FallDistance)) $this->namedtag->FallDistance = new FloatTag("FallDistance", 0.0);
		$this->fallDistance = $this->namedtag["FallDistance"];

		if(!isset($this->namedtag->Fire)) $this->namedtag->Fire = new ShortTag("Fire", 0);
		$this->fireTicks = $this->namedtag["Fire"];
		if($this->isOnFire()) $this->setGenericFlag(self::DATA_FLAG_ONFIRE);
	...
	public function setScale(float $value) : void{
		if($value <= 0) throw new \InvalidArgumentException("Scale must be greater than 0");
	...
	public function getOwningEntity(){
		...
		if($eid !== null) return $this->server->findEntity($eid);
	...
	public function setTargetEntity(Entity $target){
		if($target->closed) throw new \InvalidArgumentException("Supplied target entity is garbage and cannot be used");
	...
	public function removeEffect($effectId){
		if(isset($this->effects[$effectId])){
			...
			if($effectId === Effect::ABSORPTION and $this instanceof Human) $this->setAbsorption(0);
			if($ev->isCancelled()){
				if($hasExpired and !$ev->getEffect()->hasExpired()) $ev->getEffect()->add($this, true);
			    return false;
		    }
	...
	public function addEffect(Effect $effect){
		...
		if($effect->getId() === Effect::HEALTH_BOOST) $this->setHealth($this->getHealth() + 4 * ($effect->getAmplifier() + 1));
		if($effect->getId() === Effect::ABSORPTION and $this instanceof Human) $this->setAbsorption(4 * ($effect->getAmplifier() + 1));
	...
	protected function recalculateEffectColor(){
		...
		foreach($this->effects as $effect){
			if($effect->isVisible()){
				...
				if(!$effect->isAmbient()) $ambient = false;
			}
		}
	...
	public static function registerEntity($className, $force = false){
		...
		if(is_a($className, Entity::class, true) and !$class->isAbstract()){
			if($className::NETWORK_ID !== -1) self::$knownEntities[$className::NETWORK_ID] = $className;
	...
	public function saveNBT(){
		...
		}else unset($this->namedtag->ActiveEffects);
	}

	protected function initEntity(){
		if(!($this->namedtag instanceof CompoundTag)) throw new \InvalidArgumentException("Expecting CompoundTag, received " . get_class($this->namedtag));
		...
		if(isset($this->namedtag->CustomName)){
			...
			if(isset($this->namedtag->CustomNameAlwaysVisible)) $this->setNameTagAlwaysVisible($this->namedtag["CustomNameAlwaysVisible"] > 0);
		}
	...
	public function sendData($player, array $data = null){
		...
		if($this instanceof Player) $this->dataPacket($pk);
	}
	...
	protected function checkObstruction($x, $y, $z){
		if(sizeof($this->level->getCollisionCubes($this, $this->getBoundingBox(), false)) === 0) return false;
	...
	public function entityBaseTick($tickDiff = 1){
		...
		if($this->fireTicks > 0){
			if($this->isFireProof()){
				if($this->fireTicks > 1) $this->fireTicks = 1;
				else $this->fireTicks -= 1;
			}else{
	...
	public function onUpdate($currentTick){
		...
		if($tickDiff <= 0){
			if(!$this->justCreated) $this->server->getLogger()->debug("Expected tick difference of at least 1, got $tickDiff for " . get_class($this));
	...
	protected function updateFallState($distanceThisTick, $onGround){
		if($onGround){
			if($this->fallDistance > 0){
				if($this instanceof Living) $this->fall($this->fallDistance);
	...
	public function fall($fallDistance){
		if($this instanceof Player and $this->isSpectator()) return;
		if($fallDistance > 3) $this->getLevel()->addParticle(new DestroyBlockParticle($this, $this->getLevel()->getBlock($this->floor()->subtract(0, 1, 0))));
		...
		if($this->getLevel()->getBlock($this->floor()->subtract(0, 1, 0)) instanceof SlimeBlock) $damage = 0;
		//TODO Improve
		if($this instanceof Player){
			if($this->getInventory()->getChestplate() instanceof Elytra) $damage = 0;
		}
		if($damage > 0){
	...
	public function isInsideOfFire(){
		foreach($this->getBlocksAround() as $block){
			if($block instanceof Fire) return true;
		}
	...
	public function fastMove($dx, $dy, $dz){
		...
		if(!$this->onGround or $dy != 0){
			...
			if(count($this->level->getCollisionBlocks($bb)) > 0) $this->onGround = true;
		}
			...
			for($z = $minZ; $z <= $maxZ; ++$z){
				for($x = $minX; $x <= $maxX; ++$x){
					for($y = $minY; $y <= $maxY; ++$y){
						...
						if($block->hasEntityCollision()) $this->blocksAround[] = $block;
					}
				}
			}
		}
		...
		if($this->chunk === null or ($this->chunk->getX() !== $chunkX or $this->chunk->getZ() !== $chunkZ)){
			...
			if(!$this->justCreated){
				...
				foreach($this->hasSpawned as $player){
					if(!isset($newChunk[$player->getLoaderId()])) $this->despawnFrom($player);
	...
	public function setPosition(Vector3 $pos){
		...
		if($pos instanceof Position and $pos->level !== null and $pos->level !== $this->level){
			if(!$this->switchLevel($pos->getLevel())) return false;
		}
	...
	public function isGamePad() : bool{
		return $this->getDataFlag(Entity::DATA_FLAGS, Entity::DATA_FLAG_WASD_CONTROLLED);
	}
	...
	public function setMotion(Vector3 $motion){
		...
		if(!$this->justCreated) $this->updateMovement();
	...
	public function spawnToAll(){
		if($this->chunk === null or $this->closed) return;
		foreach($this->level->getViewersForPosition($this) as $player){
			if($player->isOnline()) $this->spawnTo($player);
		}
	}
	...
	public function close(){
		...
		if(!$this->closed){
			...
			if($this->linkedType != 0) $this->linkedEntity->setLinked(0, $this);
	...
	public function sendLinkedData(){
		if($this->linkedEntity instanceof Entity) $this->setLinked($this->linkedType, $this->linkedEntity);
	}
	...
	public function setLinked($type, Entity $entity){
		if($entity instanceof Boat or $entity instanceof Minecart or $entity->getName() == "Horse") $this->setDataProperty(57, 8, [0, 1, 0]); //This is a fast hack for Boat. TODO: Improve it

		if($type != 0 and $entity === null) return false;
		if($entity === $this) return false;
		switch($type){
			case 0:
				if($this->linkedType == 0) return true;
				$this->linkedType = 0;
				...
				if($this->linkedEntity->getLinkedType()) $this->linkedEntity->setLinked(0, $this);
			...
			case 2:
				if(!$entity->isAlive()) return false;
				if($entity->getLinkedEntity() !== $this) return $entity->linkEntity($this);
				$this->linkedEntity = $entity;
				$this->linkedType = 2;
