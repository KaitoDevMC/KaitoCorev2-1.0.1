...
class Effect {
	...
	public static function getEffect($id){
		if(isset(self::$effects[$id])) return clone self::$effects[(int) $id];
		return null;
	}
	...
	public function canTick(){
		...
		switch($this->id){
			case Effect::POISON:
				if(($interval = (25 >> $this->amplifier)) > 0) return ($this->duration % $interval) === 0;
				return true;
			case Effect::WITHER:
				if(($interval = (50 >> $this->amplifier)) > 0) return ($this->duration % $interval) === 0;
				return true;
			case Effect::REGENERATION:
				if(($interval = (40 >> $this->amplifier)) > 0) return ($this->duration % $interval) === 0;
				return true;
			case Effect::HUNGER:
				return true;
			case Effect::HEALING:
			...
			case Effect::SATURATION:
				...
				if(($interval = (20 >> $this->amplifier)) > 0) return ($this->duration % $interval) === 0;
				return true;
		}
		return false;
	}
	...
	public function applyEffect(Entity $entity){
		switch($this->id){
			...
			case Effect::REGENERATION:
				...
				break;
			case Effect::HUNGER:
				if($entity instanceof Human) $entity->exhaust(0.025 * $this->getEffectLevel(), PlayerExhaustEvent::CAUSE_POTION);
				break;
			case Effect::HEALING:
			...
			case Effect::HARMING:
				...
				if(($entity->getHealth() - 6 * $level) >= 0){
					$ev = new EntityDamageEvent($entity, EntityDamageEvent::CAUSE_MAGIC, 1 * $level);
					$entity->attack($ev->getFinalDamage(), $ev);
				}else{
			...
			case Effect::SATURATION:
				if($entity instanceof Player){
					if($entity->getServer()->foodEnabled) $entity->setFood($entity->getFood() + 1);
				}
				break;
		}
	}
	...
	public function add(Entity $entity, $modify = false, Effect $oldEffect = null){
		if($entity instanceof Player){
			...
			if($modify) $pk->eventId = MobEffectPacket::EVENT_MODIFY;
			else $pk->eventId = MobEffectPacket::EVENT_ADD;

			$entity->dataPacket($pk);

			if($this->id === Effect::SPEED){
				$attr = $entity->getAttributeMap()->getAttribute(Attribute::MOVEMENT_SPEED);
				if($modify and $oldEffect !== null) $speed = $attr->getValue() / (1 + 0.2 * ($oldEffect->getEffectLevel()));
				else $speed = $attr->getValue();
				$speed *= (1 + 0.2 * ($this->getEffectLevel()));
				$attr->setValue($speed);
			}elseif($this->id === Effect::SLOWNESS){
				$attr = $entity->getAttributeMap()->getAttribute(Attribute::MOVEMENT_SPEED);
				if($modify and $oldEffect !== null) $speed = $attr->getValue() / (1 - 0.15 * ($oldEffect->getEffectLevel()));
				else $speed = $attr->getValue();
