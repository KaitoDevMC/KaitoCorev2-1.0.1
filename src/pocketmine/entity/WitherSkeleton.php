	...
	public function getDrops(){
		...
		if($cause instanceof EntityDamageByEntityEvent){
			...
			if($damager instanceof Player){
				...
				if(mt_rand(1, 200) <= (5 + 2 * $lootingL)) $drops[] = ItemItem::get(ItemItem::SKULL, 1, 1);
				$drops[] = ItemItem::get(ItemItem::BONE, 0, mt_rand(0, 2));
				$drops[] = ItemItem::get(ItemItem::COAL, 0, mt_rand(0, 1));
				if(mt_rand(1, 3) === 3) $drops[] = ItemItem::get(ItemItem::STONE_SWORD, mt_rand(30, 120), 1);
			...
			if($this->fallDistance > 0){
				...
				if($this->willMove()){
					...
					if(($this->motionVector == null)or($this->distance($this->motionVector) < $this->step)){
						...
						if(($this->motionVector->x - $this->x) > $this->step) $x = $this->step;
						elseif(($this->motionVector->x - $this->x) < -$this->step) $x = -$this->step;
						if(($this->motionVector->z - $this->z) > $this->step) $z = $this->step;
						elseif(($this->motionVector->z - $this->z) < -$this->step) $z = -$this->step;
