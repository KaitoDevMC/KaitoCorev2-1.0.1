...
class Vector3{
	...
	public function add($x, $y = 0, $z = 0){
		...
		else return new Vector3($this->x + $x, $this->y + $y, $this->z + $z);
	}
	...
	public function subtract($x, $y = 0, $z = 0){
		if($x instanceof Vector3) return $this->add(-$x->x, -$x->y, -$x->z);
		else return $this->add(-$x, -$y, -$z);
	}
	...
	public function maxPlainDistance($x, $z = 0) : float{
		...
		}else return max(abs($this->x - $x), abs($this->z - $z));
	}
	...
	public function normalize(){
		...
		if($len > 0) return $this->divide(sqrt($len));
	...
	public function getIntermediateWithXValue(Vector3 $v, $x){
		...
		if($f < 0 or $f > 1) return null;
		else return new Vector3($x, $this->y + ($v->y - $this->y) * $f, $this->z + ($v->z - $this->z) * $f);
	}
	...
	public function getIntermediateWithYValue(Vector3 $v, $y){
		...
		if($f < 0 or $f > 1) return null;
		else return new Vector3($this->x + ($v->x - $this->x) * $f, $y, $this->z + ($v->z - $this->z) * $f);
	}
	...
	public function getIntermediateWithZValue(Vector3 $v, $z){
		...
		if($f < 0 or $f > 1) return null;
		else return new Vector3($this->x + ($v->x - $this->x) * $f, $this->y + ($v->y - $this->y) * $f, $z);
	}
