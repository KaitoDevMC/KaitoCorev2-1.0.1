...
class BaseLang {
	...
	public function __construct($lang, $path = null, $fallback = self::FALLBACK_LANGUAGE){
		...
		if($path === null) $path = \pocketmine\PATH . "src/pocketmine/lang/locale/";
	...
	public function translate(TextContainer $c){
		...
		}else $baseText = $this->parseTranslation($c->getText());
	...
	public function internalGet($id){
		if(isset($this->lang[$id])) return $this->lang[$id];
		elseif(isset($this->fallbackLang[$id])) return $this->fallbackLang[$id];
		...
		for($i = 0; $i < $len; ++$i){
			...
			if($replaceString !== null){
				...
				if(
					...
					if(($t = $this->internalGet(substr($replaceString, 1))) !== null and ($onlyPrefix === null or strpos($replaceString, $onlyPrefix) === 1)) $newString .= $t;
					else $newString .= $replaceString;
					$replaceString = null;

					if($c === "%") $replaceString = $c;
					else $newString .= $c;
				}
			}elseif($c === "%"){
				$replaceString = $c;
