...
class TimingsHandler {
	...
	public static function printTimings($fp){
		...
		foreach(self::$HANDLERS as $timings){
			...
			fwrite($fp, "    " . $timings->name . " Time: " . round($time * 1000000000) . " Count: " . $count . " Avg: " . round($time / $count * 1000000000) . " Violations: " . $timings->violations . PHP_EOL);
		}
		...
		foreach(Server::getInstance()->getLevels() as $level){
			...
			foreach($level->getEntities() as $e){
				if($e instanceof Living) ++$livingEntities;
			}
		}
	...
	public static function reload(){
		if(Server::getInstance()->getPluginManager()->useTimings()){
			foreach(self::$HANDLERS as $timings) $timings->reset();
	...
	public static function tick($measure = true){
		if(PluginManager::$useTimings){
			if($measure){
				foreach(self::$HANDLERS as $timings){
					if($timings->curTickTotal > 0.05) $timings->violations += round($timings->curTickTotal / 0.05);
	...
	public function startTiming(){
		if(PluginManager::$useTimings and ++$this->timingDepth === 1){
			...
			if($this->parent !== null and ++$this->parent->timingDepth === 1) $this->parent->start = $this->start;
		}
	}

	public function stopTiming(){
		if(PluginManager::$useTimings){
			if(--$this->timingDepth !== 0 or $this->start === 0) return;
			...
			++$this->count;
			$this->start = 0;
			if($this->parent !== null) $this->parent->stopTiming();
		}
	}
