...
abstract class Timings{
	...
	public static function getPluginTaskTimings(TaskHandler $task, $period){
		...
		$name = "Task: " . $plugin . " Runnable: " . $task->getTaskName();

		if($period > 0) $name .= "(interval:" . $period . ")";
		else $name .= "(Single)";

		if(!isset(self::$pluginTaskTimingMap[$name])) self::$pluginTaskTimingMap[$name] = new TimingsHandler($name, self::$schedulerSyncTimer);
	...
	public static function getEntityTimings(Entity $entity){
		...
		if(!isset(self::$entityTypeTimingMap[$entityType])){
			if($entity instanceof Player) self::$entityTypeTimingMap[$entityType] = new TimingsHandler("** tickEntity - EntityPlayer", self::$tickEntityTimer);
			else self::$entityTypeTimingMap[$entityType] = new TimingsHandler("** tickEntity - " . $entityType, self::$tickEntityTimer);
		}
	...
	public static function getTileEntityTimings(Tile $tile){
		...
		if(!isset(self::$tileEntityTypeTimingMap[$tileType])) self::$tileEntityTypeTimingMap[$tileType] = new TimingsHandler("** tickTileEntity - " . $tileType, self::$tickTileEntityTimer);
