...
class EnchantInventory extends TemporaryInventory {
	...
	public function onOpen(Player $who){
		...
		if($this->levels == null){
			...
			if($this->bookshelfAmount < 0) $this->bookshelfAmount = 0;

			if($this->bookshelfAmount > 15) $this->bookshelfAmount = 15;
		...
		if(!$before->hasEnchantments() and $after->hasEnchantments() and $after->getId() === $result->getId() and $this->levels !== null and $this->entries !== null){
			...
			for($i = 0; $i < 3; ++$i){
				if($this->checkEnts($enchantments, $this->entries[$i]->getEnchantments())){
					...
					if($lapis->getId() === Item::DYE and $lapis->getDamage() === Dye::BLUE and $lapis->getCount() > $i and $who->getXpLevel() >= $this->entries[$i]->getCost()){
		...
		if($this->getHolder()->getLevel()->getServer()->countBookshelf){
			...
			for($i = 0; $i < 3; $i++){
				foreach($offsets as $offset){
					if($pos->getLevel()->getBlockIdAt($pos->x + $offset[0], $pos->y + $i, $pos->z + $offset[1]) == Block::BOOKSHELF) ++$count;
					if($count >= 15) break 2;
				}
			}
			return $count;
		}else return mt_rand(0, 15);
	}
