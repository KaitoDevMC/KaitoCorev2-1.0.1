...
class PlayerInventory extends BaseInventory{
				...
				foreach($contents as $item){
					if($item["Slot"] >= 0 and $item["Slot"] < $this->getHotbarSize()){ //Hotbar
						if(isset($item["TrueSlot"])){
							...
							}elseif($item["TrueSlot"] < 0) $this->hotbar[$item["Slot"]] = -1;
						}
					...
					}else $this->setItem($item["Slot"] - $this->getHotbarSize(), Item::nbtDeserialize($item), false);
				}
	...
	public function setHotbarSlotIndex($index, $slot){
		if($this->getHolder()->getServer()->getProperty("settings.deprecated-verbose") !== false) trigger_error("Do not attempt to change hotbar links in plugins!", E_USER_DEPRECATED);
	}
	...
	public function setHeldItemIndex($hotbarSlotIndex, $sendToHolder = true, $slotMapping = null){
		if($slotMapping !== null) $slotMapping -= $this->getHotbarSize();
		if(0 <= $hotbarSlotIndex and $hotbarSlotIndex < $this->getHotbarSize()){
			...
			if($slotMapping !== null){
				...
				if($slotMapping < 0 or $slotMapping >= $this->getSize()) $slotMapping = -1;
				...
				if(($key = array_search($slotMapping, $this->hotbar)) !== false and $slotMapping !== -1) $this->hotbar[$key] = $this->hotbar[$this->itemInHandIndex];

				$this->hotbar[$this->itemInHandIndex] = $slotMapping;
			}
			$this->sendHeldItem($this->getHolder()->getViewers());
			if($sendToHolder) $this->sendHeldItem($this->getHolder());
		}
	}
	...
	public function sendHeldItem($target){
		...
		if(!is_array($target)){
			...
			if($this->getHeldItemSlot() !== -1 and $target === $this->getHolder()) $this->sendSlot($this->getHeldItemSlot(), $target);
		}else{
			$this->getHolder()->getLevel()->getServer()->broadcastPacket($target, $pk);
			if($this->getHeldItemSlot() !== -1 and in_array($this->getHolder(), $target, true)) $this->sendSlot($this->getHeldItemSlot(), $this->getHolder());
		}
	}
	...
	public function onSlotChange($index, $before, $send){
		if($send){
			...
			if(!$holder instanceof Player or !$holder->spawned) return;
		...
		if($index === $this->itemInHandIndex){
			$this->sendHeldItem($this->getHolder()->getViewers());
			if($send) $this->sendHeldItem($this->getHolder());
	...
	public function damageArmor($index, $cost){
		...
		if($this->slots[$itemIndex]->getDamage() >= $this->slots[$itemIndex]->getMaxDurability()) $this->setItem($itemIndex, Item::get(Item::AIR, 0, 0));
	...
	public function setItem($index, Item $item, $send = true){
		...
		elseif($item->getId() === 0 or $item->getCount() <= 0) return $this->clear($index, $send);
		...
		if(isset($this->slots[$index])){
			...
			if($index >= $this->getSize() and $index < $this->size){ //Armor change
				...
				if($ev->isCancelled()){
					...
					else $this->sendSlot($index, $this->getViewers());
					return false;
				}
				$item = $ev->getNewItem();
			}else{
			...
			if($item->getId() !== Item::AIR) $this->slots[$index] = clone $item;
			else unset($this->slots[$index]);
	...
	public function setArmorContents(array $items){
		for($i = 0; $i < 4; ++$i){
			if(!isset($items[$i]) or !($items[$i] instanceof Item)) $items[$i] = Item::get(Item::AIR, 0, 0);

			if($items[$i]->getId() === Item::AIR) $this->clear($this->getSize() + $i);
			else $this->setItem($this->getSize() + $i, $items[$i]);
		}
	}
