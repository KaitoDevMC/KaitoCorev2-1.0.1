...
abstract class BaseInventory implements Inventory {
	...
	public function setContents(array $items, bool $send = true){
		if(sizeof($items) > $this->size) $items = array_slice($items, 0, $this->size, true);
		...
		for($i = 0; $i < $this->size; ++$i){
			if(!isset($items[$i])){
				...
				if(!$this->setItem($i, $items[$i], $send)) $this->clear($i, $send);
			}
		}

		if($send) $this->sendContents($this->getViewers());
	}
	...
	public function dropContents(Level $level, Vector3 $position) : void{
		foreach($this->getContents() as $item) $level->dropItem($position, $item);
	...
	public function all(Item $item){
		...
		foreach($this->getContents() as $index => $i){
			if($item->equals($i, $checkDamage, $checkTags)) $slots[$index] = $i;
		}
	...
	public function first(Item $item){
		...
		foreach($this->getContents() as $index => $i){
			if($item->equals($i, $checkDamage, $checkTags) and $i->getCount() >= $count) return $index;
		}
	...
	public function firstEmpty(){
		for($i = 0; $i < $this->size; ++$i){
			if($this->getItem($i)->getId() === Item::AIR) return $i;
		}
	...
	public function firstOccupied(){
		for($i = 0; $i < $this->size; $i++){
			if(($item = $this->getItem($i))->getId() !== Item::AIR and $item->getCount() > 0) return $i;
		}
		return -1;
	}
	...
	public function canAddItem(Item $item){
		...
		for($i = 0; $i < $this->getSize(); ++$i){
			$slot = $this->getItem($i);
			if($item->equals($slot)){
				if(($diff = $slot->getMaxStackSize() - $slot->getCount()) > 0) $item->setCount($item->getCount() - $diff);
			}elseif($slot->getId() === Item::AIR) $item->setCount($item->getCount() - $this->getMaxStackSize());
	...
	public function addItem(...$slots){
		...
		foreach($slots as $slot){
			if(!($slot instanceof Item)) throw new \InvalidArgumentException("Expected Item, got " . gettype($slot));
			if($slot->getId() !== 0 and $slot->getCount() > 0) $itemSlots[] = clone $slot;
		}
		...
		for($i = 0; $i < $this->getSize(); ++$i){
			...
			if(sizeof($itemSlots) === 0){
				break;
			}
		}

		if(sizeof($itemSlots) > 0 and sizeof($emptySlots) > 0){
