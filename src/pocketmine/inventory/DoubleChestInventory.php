   ...
   /**
	...
	public function getContents(bool $includeEmpty = false) : array{
		...
		foreach($this->slots as $i => $slot){
			if($slot !== null) $contents[$i] = clone $slot;
			elseif($includeEmpty) $contents[$i] = $air ?? ($air = Item::get(Item::AIR, 0, 0));
		}
	...
	public function setContents(array $items, $send = true){
		if(count($items) > $this->size) $items = array_slice($items, 0, $this->size, true);


		for($i = 0; $i < $this->size; ++$i){
			if(!isset($items[$i])){
				if($i < $this->left->size){
					if(isset($this->left->slots[$i])) $this->clear($i);
				}elseif(isset($this->right->slots[$i - $this->left->size])) $this->clear($i);
			}elseif(!$this->setItem($i, $items[$i])) $this->clear($i);
		}
	}
	...
	public function onOpen(Player $who){
		...
		if(count($this->getViewers()) === 1){
			...
			$pk->case1 = 1;
			$pk->case2 = 2;
			if(($level = $this->right->getHolder()->getLevel()) instanceof Level) $level->addChunkPacket($this->right->getHolder()->getX() >> 4, $this->right->getHolder()->getZ() >> 4, $pk);
		}
	}
