...
class SimpleTransactionQueue implements TransactionQueue {
	...
	public function execute(){
		...
		while(!$this->transactionsToRetry->isEmpty()) $this->transactionQueue->enqueue($this->transactionsToRetry->dequeue());
		if(!$this->transactionQueue->isEmpty()) $this->player->getServer()->getPluginManager()->callEvent($ev = new InventoryTransactionEvent($this));
		else return;
		while(!$this->transactionQueue->isEmpty()){
			$transaction = $this->transactionQueue->dequeue();

			if($transaction->getInventory() instanceof \pocketmine\inventory\ContainerInventory || $transaction->getInventory() instanceof \pocketmine\inventory\PlayerInventory){
				Server::getInstance()->getPluginManager()->callEvent($event = new \pocketmine\event\inventory\InventoryClickEvent($transaction->getInventory(), $this->player, $transaction->getSlot(), $transaction->getInventory()->getItem($transaction->getSlot())));
				if($event->isCancelled()) $ev->setCancelled();
			}
