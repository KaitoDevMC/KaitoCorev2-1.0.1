...
class RCON{
	...
	public function __construct(Server $server, string $password, int $port = 19132, string $interface = "0.0.0.0", int $maxClients = 50){
		...
		if($socket === false) throw new \RuntimeException("Failed to create socket:" . trim(socket_strerror(socket_last_error())));
		$this->socket = $socket;

		if(!socket_set_option($this->socket, SOL_SOCKET, SO_REUSEADDR, 1)) throw new \RuntimeException("Unable to set option on socket: " . trim(socket_strerror(socket_last_error())));

		if(!@socket_bind($this->socket, $interface, $port) or !@socket_listen($this->socket, 5)) throw new \RuntimeException(trim(socket_strerror(socket_last_error())));
		...
		if(!$ret){
			...
			if(($err !== SOCKET_EPROTONOSUPPORT and $err !== SOCKET_ENOPROTOOPT) or !@socket_create_pair(AF_INET, SOCK_STREAM, 0, $ipc)) throw new \RuntimeException(trim(socket_strerror(socket_last_error())));
		}
	...
	public function check(){
		...
		$this->server->getPluginManager()->callEvent($ev = new RemoteServerCommandEvent($response, $this->instance->cmd));
