...
class RCONInstance extends Thread{
	...
	private function readPacket($client, ?int &$requestID, ?int &$packetType, ?string &$payload){
		...
		if($d === false){
			...
			if($err !== SOCKET_ECONNRESET) $this->logger->debug('Connection error with '.$ip.' '.$port.': ' . trim(socket_strerror($err)));
			return false;
		}
		if(strlen($d) !== 4){
			if($d !== "") $this->logger->debug("Truncated packet from $ip $port (want 4 bytes, have " . strlen($d) . "), disconnecting");
			return false;
		}
