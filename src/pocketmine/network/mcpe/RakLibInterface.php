...
class RakLibInterface implements ServerInstance, AdvancedSourceInterface{
	...
	public function __construct(Server $server){
		...
		if($isEnable) {
			$this->packetLogger = new RaklibPacketLogger($this->server->getDataPath()."logs".DIRECTORY_SEPARATOR);
		    $this->packetLogger->start(PTHREADS_INHERIT_NONE);
		}else $logger = $this->server->getLogger();
	...
	public function process() : void{
		...
		if(!$this->rakLib->isRunning() and !$this->rakLib->isShutdown()) throw new \Exception("RakLib Thread crashed");
	}
	...
	public function shutdown(){
		...
		if($this->packetLogger !== null) $this->packetLogger->shutdown();
	}
		...
		if(isset($this->players[$identifier])){
			...
			try{
				if($packet->buffer !== ""){
					...
					if($this->packetLogger !== null) $this->packetLogger->putPacket($address, $packet->buffer);
	...
	private function getPacket($buffer){
		$pid = ord($buffer[0]);
		if(($data = $this->network->getPacket($pid)) === null) return null;
