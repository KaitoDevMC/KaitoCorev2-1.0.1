...
class LoginPacket extends DataPacket {
	...
	public function decode(){
		...
		if(!in_array($this->protocol, ProtocolInfo::ACCEPTED_PROTOCOLS)) return $this->buffer = null;
		...
		while(!empty($chainData)){
			foreach($chainData as $index => $chain){
				...
				if(isset($webtoken['extraData'])){
					if(isset($webtoken['extraData']['displayName'])) $this->username = $webtoken['extraData']['displayName'];
					if(isset($webtoken['extraData']['identity'])) $this->clientUUID = $webtoken['extraData']['identity'];
				}
				if($verified) $verified = isset($webtoken['nbf']) && $webtoken['nbf'] <= $time && isset($webtoken['exp']) && $webtoken['exp'] > $time;
				if($verified and isset($webtoken['identityPublicKey'])){
					...
					break;
				}elseif($chainKey === null) break;
			}
			if(!$verified && $chainKey !== null) $chainKey = null;
			else unset($chainData[$index]);
		...
		if(isset($this->clientData['SkinData'])) $this->skin = base64_decode($this->clientData['SkinData']);

		if(isset($this->clientData['LanguageCode'])) $this->languageCode = $this->clientData['LanguageCode'];

		if(isset($this->clientData['DeviceModel'])) $this->deviceModel = $this->clientData['DeviceModel'];

		if(isset($this->clientData['DeviceOS'])) $this->deviceOS = $this->clientData['DeviceOS'];

		if($verified) $this->identityPublicKey = $chainKey;
	}
	...
	public function decodeToken($token, $key){
		...
		if($key !== null and extension_loaded('openssl')){
			...
			$len = 2 + $j + 2 + $l;
			$derSig = chr(48);
			if($len > 255) throw new \RuntimeException('Invalid signature format');
			elseif($len >= 128) $derSig .= chr(81);
