...
class Item implements ItemIds, JsonSerializable {
	...
	protected static function parseCompoundTag(string $tag) : CompoundTag{
		if(self::$cachedParser === null) self::$cachedParser = new NBT(NBT::LITTLE_ENDIAN);
	...
	public static function init($readFromJson = false, bool $registerCreativeItems = true){
		if(self::$list === null){
			...
			self::$list[self::LEASH] = Leash::class;
	...
	public static function removeCreativeItem(Item $item){
		...
		if($index !== -1) CreativeItemsStorage::getInstance()->removeItemByIndex($index);
	}
	...
	public static function isCreativeItem(Item $item) : bool{
		foreach(CreativeItemsStorage::getInstance()->getItems() as $i => $d){
			if($item->equals($d, !$item->isTool())) return true;
		}
	...
	public static function fromString(string $str, bool $multiple = false){
		if($multiple){
			...
			if(!isset($b[1])){
				...
				$meta = (int) $b[1];
			}else throw new InvalidArgumentException("Unable to parse \"" . $b[1] . "\" from \"" . $str . "\" as a valid meta value");

			if(is_numeric($b[0])){
				$item = self::get((int) $b[0] & 0xFFFF, $meta);
			}elseif(defined(Item::class . "::" . strtoupper($b[0]))){
				$item = self::get(constant(Item::class . "::" . strtoupper($b[0])), $meta);
			}else throw new InvalidArgumentException("Unable to resolve \"" . $str . "\" to a valid item");
	...
	public function hasCustomBlockData() : bool{
		...
		if(isset($tag->BlockEntityTag) and $tag->BlockEntityTag instanceof CompoundTag) return true;
	...
	public function getCustomBlockData(){
		...
		$tag = $this->getNamedTag();
		if(isset($tag->BlockEntityTag) and $tag->BlockEntityTag instanceof CompoundTag) return $tag->BlockEntityTag;
	...
	public function addEnchantments(array $enchantments){
		foreach($enchantments as $enchantment) $this->addEnchantment($enchantment);
	}
	/**
	 * @return Enchantment[]
	 */
	...
	public function getNamedTagEntry($name){
		$tag = $this->getNamedTag();
		if($tag !== null) return isset($tag->{$name}) ? $tag->{$name} : null;
