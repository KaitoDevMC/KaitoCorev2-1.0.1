...
abstract class Armor extends Item {
	...
	public function useOn($object, int $cost = 1){
		if($this->isUnbreakable()) return true;
		$unbreakings = [
			0 => 100,
			1 => 80,
			2 => 73,
		...
		if(mt_rand(1, 100) > $unbreakings[$unbreakingl]) return true;
		$this->setDamage($this->getDamage() + $cost);
		if($this->getDamage() >= $this->getMaxDurability()) $this->setCount(0);
		return true;
	}
	...
	public function setCustomColor(Color $color){
		if(($hasTag = $this->hasCompoundTag())) $tag = $this->getNamedTag();
	...
	public function getCustomColor(){
		...
		if(isset($tag->customColor)) return $tag["customColor"];
		return null;
	}
	...
	public function clearCustomColor(){
		...
		$tag = $this->getNamedTag();
		if(isset($tag->customColor)) unset($tag->customColor);
		$this->setNamedTag($tag);
	}
