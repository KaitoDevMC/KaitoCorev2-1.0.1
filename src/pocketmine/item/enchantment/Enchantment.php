...
class Enchantment {
	...
	public static function init(){
		...
		self::$enchantments[self::TYPE_MENDING] = new Enchantment(self::TYPE_MENDING, "%enchantment.mending.all", self::RARITY_MYTHIC, self::ACTIVATION_EQUIP, self::SLOT_ALL);
		self::$enchantments[self::TYPE_FROST_WALKER] = new Enchantment(self::TYPE_FROST_WALKER, "%enchantment.frost_walker.all", self::RARITY_MYTHIC, self::ACTIVATION_EQUIP, self::SLOT_FEET);
	...
	public static function get($id){
		if(isset(self::$enchantments[$id])) return clone self::$enchantments[(int) $id];
		return new Enchantment(self::TYPE_INVALID, "unknown", 0, 0, 0);
	}
	/**
	 * @param $id
	...
	public static function getEnchantWeight(int $enchantmentId){
		switch($enchantmentId){
			...
			case self::TYPE_FROST_WALKER:
				return 2;
			case self::TYPE_BOW_FLAME:
				return 2;
			case self::TYPE_MENDING:
				return 1;
	...
	public function equals(Enchantment $ent){
		if($ent->getId() == $this->getId() and $ent->getLevel() == $this->getLevel() and $ent->getActivationType() == $this->getActivationType() and $ent->getRarity() == $this->getRarity()) return true;
		return false;
	}
