...
abstract class Packet{
	...
	protected function getAddress(&$addr, &$port, &$version = null){
		...
		}else throw new \UnexpectedValueException('Unknown IP address version '.$version);
	}
	...
	protected function putAddress($addr, $port, $version = 4){
		...
		if($version === 4){
			foreach(explode(".", $addr) as $b) $this->putByte((~((int) $b)) & 0xff);
			$this->putShort($port);
		}elseif($version === 6){
			...
			$this->putInt(0);
		}else throw new \InvalidArgumentException("IP version $version is not supported");
	}
